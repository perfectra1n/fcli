...
fn null_slice_decodes_as_empty() {
    ...
    inst.fjo(["issue", "view", "1", "-R", &repo.slug()]).assert_ok("fjo issue view");

    let run = inst.fjo(["issue", "list", "-R", &repo.slug(), "--json", "number,assignees"]);
    run.assert_ok("fjo issue list --json assignees");
    assert_eq!(
...
fn open_pull_request_decodes() {
    ...
    inst.fjo(["pr", "list", "-R", &repo.slug()]).assert_ok("fjo pr list with an open PR");
    inst.fjo(["pr", "view", "1", "-R", &repo.slug()]).assert_ok("fjo pr view on an open PR");
}
...
fn merged_pull_request_decodes() {
    ...
    inst.fjo(["pr", "view", "1", "-R", &repo.slug()]).assert_ok("fjo pr view on a merged PR");
}
...
/// (`name: ""`, `type: ""`). `fjo workflow list` filters on `type == "file"`, discards the
...
fn directory_listing_decodes() {
    ...
    let run = inst.fjo(["workflow", "list", "-R", &repo.slug()]);
    run.assert_ok("fjo workflow list");
    assert!(
        run.stdout.contains("ci.yml"),
        "the API lists ci.yml but `fjo workflow list` reported nothing. Its exit status was 0, \
