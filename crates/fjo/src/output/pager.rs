...
//!
//! Precedence is `FJO_PAGER` → `PAGER` → the config file's `pager` value. The tool-specific
//! variable comes first so that `FJO_PAGER=cat fjo pr list` disables paging for one command
...
pub fn resolve(env: &dyn Env, term: &Term, config: Option<&str>) -> Option<Vec<String>> {
    ...
    let raw =
        env.var("FJO_PAGER").or_else(|| env.var("PAGER")).or_else(|| config.map(str::to_string))?;
...
mod tests {
    ...
    /// Bug this prevents: reading `PAGER` before `FJO_PAGER`, so a per-command
    /// `FJO_PAGER=cat` is ignored and the user cannot disable paging without editing their
    /// shell profile.
    #[test]
    fn fjo_pager_wins_over_pager_wins_over_config() {
        let env = MapEnv::new().with("FJO_PAGER", "moar").with("PAGER", "less");
