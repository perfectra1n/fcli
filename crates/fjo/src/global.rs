...
//!
//! `main` parses `argv` twice over, in two very different ways: `fjo raw …` gets a
...
//! on `fjo raw repo get`, `--repo` naming the repository path parameter is what the user
...
// `body:`, `form:`, `fjo:`) whatever the generated tables contain.
...
impl GlobalOpts {
    ...
    /// is an implementation detail, and a global can legally appear at any depth — `fjo --jq .
    /// api user` and `fjo api user --jq .` must behave identically. Merging across the levels we
...
pub fn args(taken: &BTreeSet<String>) -> Vec<Arg> {
    ...
    push(
        named(
            Arg::new(HOST)
                ...
                .help("Forgejo server to use [env: FJO_HOST, FORGEJO_HOST]"),
            "host",
            None,
            taken,
        )
    ...
    push(named(
        Arg::new(LOGIN)
            .value_name("USER")
            .action(ArgAction::Set)
            .help("Account to use on the selected host [env: FJO_USER, FORGEJO_USER]"),
        "login",
        None,
        taken,
    ));
...
mod tests {
    ...
    fn root(taken: &BTreeSet<String>) -> Command {
        augment(Command::new("fjo").no_binary_name(false), taken)
    }
    ...
    fn bare_json_arrives_as_an_empty_string() {
        ...
        assert_eq!(parse(&["fjo", "--json"]).json.as_deref(), Some(""));
        assert_eq!(parse(&["fjo", "--json", "a,b"]).json.as_deref(), Some("a,b"));
        assert_eq!(parse(&["fjo"]).json, None);
    }

    #[test]
    fn shorts_and_env_shaped_flags_round_trip() {
        let g = parse(&["fjo", "-R", "them/proj", "-q", ".[0]", "-t", "{{.}}", "--limit", "7"]);
    ...
    fn hostname_is_accepted_as_a_hidden_alias_of_host() {
        assert_eq!(parse(&["fjo", "--hostname", "x.example"]).host.as_deref(), Some("x.example"));
    }
    ...
    /// Bug this prevents: `fjo api user --jq .login` silently ignoring `--jq` because the value
    ...
    fn a_global_is_seen_wherever_on_the_line_it_appears() {
        let root = augment(
            Command::new("fjo").subcommand(Command::new("api").arg(Arg::new("e").index(1))),
            &BTreeSet::new(),
        );
        for words in [
            &["fjo", "--jq", ".login", "api", "user"][..],
