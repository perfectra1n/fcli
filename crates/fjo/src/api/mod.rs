//! Layer 1: `fjo api <endpoint>`, modelled on `gh api`.
//!
//! The escape hatch. Every endpoint on the instance is reachable here whether or not the
//! specification this build was generated from mentions it, which is what makes `fjo` useful
//! against a Forgejo newer than the pinned spec — and what makes it possible to answer "does
...
//!   `fjo api 'repos/{owner}/{repo}/pulls'` works inside a clone. Context is resolved only
//!   when a placeholder actually needs it — shelling out to `git` for `fjo api version` would
//!   be pure cost.
...
use forgejo_core::http::{Accept, Body, Client, Mime, RawResponse, Request, encode, redact};
...
pub fn run(globals: &GlobalOpts, args: &ApiArgs, matches: &ArgMatches) -> Result<()> {
    ...
    // is a usage error rather than `no Forgejo host is set up yet`. Reporting a configuration
...
impl PipelineParts {
    fn compile(globals: &GlobalOpts) -> Result<Self> {
        let fields = match globals.json.as_deref() {
            ...
            Some("") => {
                return Err(usage(
                    "fjo api cannot list fields for arbitrary endpoints. Specify --json id,name, inspect with --jq keys, or list fields with `fjo raw <group> <op> --json`."
                        .to_owned(),
                ));
            }
            Some(list) => Some(
...
fn write_response(
    ...
    if mime.is_text() {
        // Verbatim, with no trailing newline added: `fjo api repos/o/r/raw/README` is how you
...
/// so `fjo api repos/o/r/issues -f state=open --paginate` stays a GET with `state` in the query
...
///
/// `http::Method` is not a direct dependency of `fjo`, so the type cannot be *named* here — but
...
/// without `fjo` taking a dependency purely to spell one type.
...
fn endpoint_path(
    ...
    if trimmed.is_empty() {
        return Err(usage("the endpoint is empty; try `fjo api version`".to_owned()));
    }
...
/// a repository whose name contains a `/` (rejected by Forgejo, but reachable through `-R`)
...
fn substitute(endpoint: &str, rt: &Runtime, globals: &GlobalOpts) -> Result<String> {
    ...
    while let Some(open) = rest.find('{') {
        ...
        let value = match name {
            ...
            other => {
                return Err(usage(format!(
                    "{{{other}}} is not a placeholder fjo substitutes; \
...
/// `X-FORGEJO-OTP` — are attached inside the client and are not visible here at all, which is
...
///
/// Forgejo answers a few endpoints with `text/plain` and a JSON body, and one with no
...
mod tests {
    ...
    fn interleaved_field_flags_keep_their_command_line_order() {
        let (args, m) = parse(&["fjo", "x", "-F", "a=1", "-f", "b=2", "-F", "c=3"]);
    ...
    fn bare_json_on_layer_one_explains_the_alternatives() {
        ...
        assert!(msg.contains("fjo raw"), "{msg}");
