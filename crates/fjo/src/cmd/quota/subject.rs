...
//! the question `fjo quota status` exists to answer, and answering it is why this module
...
/// and refusing it would make `fjo` the reason an admin cannot use their own server. It is used
...
mod tests {
    ...
    /// `fjo quota status` reports a user comfortably inside a `size:all` limit they are in fact
