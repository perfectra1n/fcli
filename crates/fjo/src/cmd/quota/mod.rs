...
//! already tells them the cause is a quota and points them here. `fjo quota status` has to
...
If no rules appear, ask an administrator whether [quota] ENABLED is set in app.ini.
  ...
  fjo quota status                    # your usage against the rules that apply to you
  fjo quota status --org acme         # an organization's
  fjo quota rules list                # the rules that apply to you
  fjo quota groups list               # the groups you are in
  fjo quota rules list --all          # every rule on the instance (admin)";
...
Attach it to a group with `fjo quota groups add-rule <group> <rule>` to apply it.

  fjo quota rules create small --bytes 1GiB --subject size:all
  fjo quota rules create no-lfs --bytes 0 --subject size:git:lfs
  fjo quota rules create exempt --bytes unlimited --subject size:all")]
pub struct RuleCreateArgs {
    ...
    /// What it covers; repeatable. See `fjo quota --help` for the vocabulary
...
pub(crate) fn render_status(term: &Term, host: &str, who: &str, info: &QuotaInfo) -> String {
    ...
    if !term.tty {
        // TSV: one row per rule, so `fjo quota status | awk -F'\t' '$4>90'` finds the rule that
    ...
    match cmd {
        ...
        RulesCmd::Create(a) => {
            ...
            porcelain::note(
                rt.term(),
                &format!(
                    "Created rule {} ({} on {}). Apply it with `fjo quota groups add-rule <group> {}`.",
                    rule.name,
    ...
    if rules.is_empty() {
        return porcelain::empty(
            ...
            if instance_wide {
                ...
                "No quota rules apply to this account. Quotas may be disabled. Admins can list all rules with `fjo quota rules list --all`."
            },
        );
    }
...
/// make `fjo` the reason an admin cannot configure their own instance. A typo is far more likely
            ...
            porcelain::note(
                ...
                &format!(
                    "note: {s:?} is not a subject this build knows; sending it anyway. \
                     `fjo quota --help` lists the ones it does."
                ),
            );
        }
    }
    ...
    match cmd {
        ...
        GroupsCmd::Create(a) => {
            ...
            porcelain::note(
                rt.term(),
                &format!(
                    "Created group {}. Add a rule with `fjo quota groups add-rule {} <rule>`.",
    ...
    if groups.is_empty() {
        return porcelain::empty(
            ...
            if instance_wide {
                ...
                "This account has no quota groups. Admins can list all groups with `fjo quota groups list --all`."
            },
        );
    }
    if let Some(m) = machine {
    ...
    match &*e.kind {
        ErrorKind::Forbidden { .. } => Error::new(ErrorKind::Usage(
            "that quota endpoint is administrator-only. Without admin you can still see what \
             applies to you: `fjo quota status`, `fjo quota rules list`, \
             `fjo quota groups list`."
                .to_owned(),
        )),
        _ => explain(e),
    }
...
mod tests {
    ...
    /// two independent limits, so `fjo quota status` shows one 1 KiB quota twice and the reader
    ...
    fn an_admin_only_refusal_names_the_non_admin_alternatives() {
        ...
        let msg = e.to_string();
        assert!(msg.contains("fjo quota status"), "{msg}");
