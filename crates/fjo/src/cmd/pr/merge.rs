//! `fjo pr merge` — one style enum, and the server's own reason when it says no.
//!
...
and --squash are also accepted. If merging is refused, the server's reason is shown.
  ...
  fjo pr merge
  fjo pr merge 42 --squash -d
  fjo pr merge -s fast-forward-only
  fjo pr merge --auto --squash")]
pub struct Args {
    ...
    crate::runtime::block_on(async move {
        ...
        if found.pr.draft {
            support::note(
                rt.term(),
                &format!(
                    "#{} is a draft ({:?}). Mark it ready with `fjo pr ready` before merging.",
...
mod tests {
    ...
    fn the_gh_shaped_aliases_all_map_onto_the_style_enum() {
        assert_eq!(style_of(&args(&["fjo", "--squash"])), "squash");
        assert_eq!(style_of(&args(&["fjo", "--rebase"])), "rebase");
        assert_eq!(style_of(&args(&["fjo", "--merge"])), "merge");
        assert_eq!(style_of(&args(&["fjo", "--rebase-merge"])), "rebase-merge");
        assert_eq!(style_of(&args(&["fjo", "--fast-forward-only"])), "fast-forward-only");
        // `-s` and the aliases agree, and `-s` wins when it is the one given.
        assert_eq!(style_of(&args(&["fjo", "-s", "squash"])), "squash");
        assert_eq!(style_of(&args(&["fjo"])), "merge");
    }
    ...
    /// through `fjo raw` and not through here.
    #[test]
    fn every_style_the_api_knows_is_accepted() {
        for style in MergeStyle::KNOWN {
            assert_eq!(style_of(&args(&["fjo", "-s", style])), *style);
        }
