//! `fjo pr close`, `reopen`, `ready`, `comment` and `edit`.
//!
//! # `ready` is a title edit
//!
//! Forgejo has no draft field and no `ready` endpoint — a pull request is a draft when its title
//! carries a work-in-progress prefix (`WIP:` by default). So `fjo pr ready` reads the title, strips
...
Use -c/--comment to add a comment before closing.
  ...
  fjo pr close
  fjo pr close 42 -c 'superseded by #45'
  fjo pr close 42 -d")]
...
Use -b for text, -F for a file (- for stdin), or -e to open an editor.
  ...
  fjo pr comment -b 'rebased onto main'
  fjo pr comment 42 -F notes.md
  fjo pr comment 42 -e")]
...
Use --title for titles; -t is reserved for output templates.
  ...
  fjo pr edit --title 'Teach the parser about tabs'
  fjo pr edit 42 --add-label bug --remove-label needs-triage
  fjo pr edit 42 -B main
  fjo pr edit 42 -e")]
pub struct EditArgs {
    /// Pull request number, URL, or branch. Defaults to the branch you are on
    #[arg(value_name = "PR")]
    ...
    crate::runtime::block_on(async move {
        ...
        if body.is_empty() {
            return Err(Error::new(ErrorKind::Usage(
                "no changes specified; see `fjo pr edit --help` for settings".to_owned(),
            )));
        }
