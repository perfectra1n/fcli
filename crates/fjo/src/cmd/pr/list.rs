//! `fjo pr list` — the table people will look at most.
//!
...
Piped output is tab-separated. Use -L or --limit to cap the result count.
  ...
  fjo pr list
  fjo pr list -s merged -L 10
  fjo pr list -A @me --json number,title
  fjo pr list -B main -l bug")]
pub struct Args {
...
mod tests {
    ...
    fn merged_is_translated_for_the_server_and_filtered_here() {
        ...
        assert!(keep(&merged, &args(&["fjo", "-s", "merged"]), None));
        assert!(!keep(&abandoned, &args(&["fjo", "-s", "merged"]), None));
        // ...and `closed` is the complement, so the two answers are disjoint.
        assert!(!keep(&merged, &args(&["fjo", "-s", "closed"]), None));
        assert!(keep(&abandoned, &args(&["fjo", "-s", "closed"]), None));
        // `all` keeps both.
        assert!(keep(&merged, &args(&["fjo", "-s", "all"]), None));
        assert!(keep(&abandoned, &args(&["fjo", "-s", "all"]), None));
    }
    ...
    fn the_assignee_filter_looks_at_both_shapes_the_api_sends() {
        ...
        assert!(keep(&one, &args(&["fjo"]), Some("alice")));
        // Logins are compared case-insensitively, as Forgejo treats them.
        assert!(keep(&many, &args(&["fjo"]), Some("bob")));
        assert!(!keep(&one, &args(&["fjo"]), Some("bob")));
    }
    ...
    fn the_search_filter_covers_the_title_and_the_body() {
        ...
        assert!(keep(&p, &args(&["fjo", "-S", "parser"]), None));
        assert!(keep(&p, &args(&["fjo", "-S", "change 1"]), None));
        assert!(!keep(&p, &args(&["fjo", "-S", "unrelated"]), None));
    }
