//! `fjo auth switch` — change which host and login every other command defaults to.
...
or --login to resolve an ambiguous choice. The updated hosts.toml uses 0600 permissions.
  ...
  fjo auth switch                                  # pick interactively
  fjo auth switch --host git.example.org
  fjo auth switch --login ci-bot
  fjo auth switch --host git.example.org --login ci-bot";
...
pub fn run(globals: &GlobalOpts, _args: &Args) -> Result<()> {
    ...
    if logins.is_empty() {
        return Err(support::usage(format!(
            "no logins are recorded for {key}; run `fjo auth login --host {key}` first"
        )));
    }
