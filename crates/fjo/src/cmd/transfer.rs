//! `fjo transfer` — moving a repository to a new owner, as an **offer**.
//!
...
//!  ─────────────────            ─────────────
//!  fjo transfer start them  ─▶  (repository still lives at you/repo,
//!                                 marked as pending transfer to `them`)
//!                                fjo transfer accept  -R you/repo
//!                                fjo transfer reject  -R you/repo
//! ```
//!
...
pub enum Cmd {
    ...
    /// accept with `fjo transfer accept`, or refuse with `fjo transfer reject` — usually from a
    ...
    match &repo.repo_transfer {
        ...
        Some(t) => {
            ...
            table.row([
                "next step".to_owned(),
                format!(
                    "the recipient runs `fjo transfer accept -R {slug}` (or `reject`) with \
                     their own token"
                ),
            ]);
        }
