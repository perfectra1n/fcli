...
//! - stdout alone is not enough. `fjo repo delete x < /dev/null` in a CI job that allocates a
...
//!   the *real* `stdout().is_terminal()` and not [`crate::output::Term::tty`] — `FJO_FORCE_TTY`
...
//! `fjo` has to be able to tell "the user said no" from "the command broke", so the
...
///
/// Presence, not a truthy value: someone who exported `FJO_PROMPT_DISABLED=` meant to disable
/// prompting, and for a flag whose failure mode is a hung command, erring towards "fail fast" is
/// the safe direction.
pub fn prompting_disabled() -> bool {
    std::env::var_os("FJO_PROMPT_DISABLED").is_some()
}
...
/// only `term.tty`, which meant `fjo webhook delete 4 < /dev/null` on a TTY-allocating CI runner
