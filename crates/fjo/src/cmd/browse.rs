//! `fjo browse` — open the repository, or a thing in it, in a browser.
//!
...
//!   usable over SSH, in a container, and in a script (`xdg-open $(fjo browse -n)`), and it is
...
//!   target below is a documented Forgejo web route, so building them locally means `fjo browse`
...
///
/// Shared with `fjo run view --web` (and anything else that grows a `-w/--web`) so that the
...
mod tests {
    ...
    const ROOT: &str = "https://git.example.org/perf3ct/fjo";
    ...
    fn a_path_is_anchored_to_a_branch_a_commit_and_a_line() {
        let a = args(Some("crates/fjo/src/main.rs"));
        assert_eq!(
            url(&a, &Ref::Branch("main".into())),
            format!("{ROOT}/src/branch/main/crates/fjo/src/main.rs")
        );
        assert_eq!(
            url(&a, &Ref::Commit("deadbeef".into())),
            format!("{ROOT}/src/commit/deadbeef/crates/fjo/src/main.rs")
        );
    ...
    /// branch) for a target that does not need one — `fjo browse --releases` must be offline.
    #[test]
