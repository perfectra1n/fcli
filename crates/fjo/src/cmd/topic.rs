//! `fjo topic` — a repository's topics.
//!
...
//! values — reachable today through `fjo raw repo update-topics`.
//!
...
//!    `fjo topic add good bad` without this check adds `good`, then fails — leaving the
...
/// that `--json topics` and `fjo raw repo list-topics --json topics` agree.
...
///
/// Normalisation is `trim` then `to_lowercase`, matching the server: `fjo topic add Rust` and
/// `fjo topic add rust` are the same request, and echoing back `Rust` when the repository now
