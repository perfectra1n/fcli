...
//! * **A bare number is refused.** Go accepts only `0`. `fjo times add 42 90` could plausibly
...
//!   `fjo times list --json time` into `fjo times add`.
...
pub fn parse_nanos(input: &str) -> Result<i128> {
    ...
    if text.starts_with('-') {
        return Err(bad(
            input,
            "tracked time cannot be negative; to remove an entry use `fjo times delete`, or \
             `fjo times reset` to clear an issue",
        ));
    }
...
mod tests {
    ...
    /// human's is minutes, so `fjo times add 42 90` is ambiguous by a factor of 60 — and both
    ...
    /// `fjo times list` into `fjo times add` would hit it, and so would every doc example.
    #[test]
