//! `fjo repo list` — repositories for you, a user, or an organization.
//!
...
use forgejo_core::{ErrorKind, Result};
...
Columns: name, description, visibility, and last update. Piped output is TSV.
  ...
  fjo repo list
  fjo repo list forgejo --source -L 100
  fjo repo list | cut -f1")]
pub struct Args {
...
async fn fetch(
    ...
    match owner {
        None => {
            let query = forgejo_client::query::UserCurrentListReposQuery::default();
        ...
        Some(owner) => {
            ...
            let query = forgejo_client::query::UserListReposQuery::default();
            ...
            match collect(&mut stream, args, limit, &mut out).await {
                ...
                {
                    out.clear();
                    let query = forgejo_client::query::OrgListReposQuery::default();
...
async fn collect(
    stream: &mut forgejo_core::http::ItemStream<Repository>,
    args: &Args,
    limit: usize,
...
mod tests {
    ...
    fn filters_are_independent_and_all_apply() {
        ...
        assert!(keep(&plain, &args(&["fjo"])));
        assert!(!keep(&plain, &args(&["fjo", "--fork"])));
        assert!(keep(&forked, &args(&["fjo", "--fork"])));
        assert!(!keep(&forked, &args(&["fjo", "--source"])));
        assert!(keep(&hidden, &args(&["fjo", "--private"])));
        assert!(!keep(&hidden, &args(&["fjo", "--public"])));
        assert!(!keep(&old, &args(&["fjo", "--no-archived"])));
        assert!(keep(&old, &args(&["fjo", "--archived"])));
        assert!(keep(&plain, &args(&["fjo", "--topic", "CLI"])), "topics match case-insensitively");
        assert!(!keep(&plain, &args(&["fjo", "--topic", "python"])));
    }
