//! The one thing `fjo admin` needs that no other group does: a 403 that names `write:admin`.
...
/// Every command under `fjo admin` therefore passes its errors through here.
///
