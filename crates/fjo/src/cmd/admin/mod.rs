//! `fjo admin` — instance administration.
//!
//! This group has **no equivalent in `gh` at all**, and `tea` covers only `tea admin users`. It is
//! the largest single capability gap between `fjo` and both of them, so it is written for the
...
//!
//! No command in this group touches [`Runtime::repo`], so `fjo admin user create` works from
//! `/tmp`. That is not incidental — an operator is usually not standing in a clone of the
//! repository they are fixing.
//!
//! # Where `admin` stops and `fjo quota` starts
//!
//! `fjo quota` (another group) is the **read** side: what a user's quota is, what they are using.
//! `fjo admin quota` is the **write** side: creating rules and groups, and putting users in them.
//! Both helps say so, because "why is there no `fjo quota rule create`?" is otherwise a fair
//! question.
//!
//! # Not implemented, for lack of an endpoint
//!
//! User **badges** have no route in the API description this build was generated from
//! (`spec/forgejo-v16.0.4.json` mentions `badge` nowhere), so `fjo admin badge` does not exist.
//! When a Forgejo release adds it, `fjo raw search badge` will find it before this group does.
...
pub enum Cmd {
    ...
    /// Storage quota rules and groups (the write side; `fjo quota` reads)
    #[command(subcommand)]
    Quota(quota::Cmd),
}
