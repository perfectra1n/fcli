//! `fjo admin adopt` — git directories on disk that the instance has no record of.
//!
...
#[derive(Debug, ClapArgs)]
pub struct One {
    /// The directory, as `fjo admin adopt list` prints it: owner/name
...
fn split(path: &str) -> Result<(&str, &str)> {
    ...
    match trimmed.split_once('/') {
        ...
        _ => Err(support::usage(format!(
            "{path:?} is not an owner/name pair; `fjo admin adopt list` prints them in exactly \
