//! `fjo admin quota` — the **write** side of Forgejo's storage quotas.
//!
...
//!
//! # This is the write half; `fjo quota` is the read half
//!
//! `fjo quota` (a different group) answers "what is my quota and what am I using" with an ordinary
//! token. Everything here needs `write:admin` and changes the instance. The split is by *privilege*
//! rather than by noun, so that `fjo quota` is safe to hand to anyone.
//!
        ...
        Rule::Create(a) => {
            ...
            emit.done(&format!(
                "created quota rule {} ({}). It enforces nothing until a group using it has \
                 members — see `fjo admin quota group add-rule`.",
                created.name,
                ...
                support::note(
                    ...
                    &format!(
                        "note: {} has no rules, so it limits nothing yet — attach one with \
                         `fjo admin quota group add-rule {} <RULE>`",
