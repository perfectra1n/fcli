...
//!   the right checkout — would be pure ceremony. `fjo stopwatch stop` works from a home
...
`stop`, `cancel`, and `status` work outside a checkout.
  ...
  fjo stopwatch start 42   # begin timing issue #42 of this repository
  fjo stopwatch status     # what is running, and for how long
  fjo stopwatch stop       # stop it and record the elapsed time
  fjo stopwatch cancel     # stop it and discard the elapsed time";
...
Use `fjo stopwatch stop` to save the time instead.")]
...
fn already_running(running: &StopWatch) -> Error {
    Error::new(ErrorKind::Usage(format!(
        "a stopwatch is already running on {} ({} so far), and Forgejo allows only one at a \
         time. Stop it and record the time with `fjo stopwatch stop`, or discard it with \
         `fjo stopwatch cancel`.",
        label(running),
        elapsed(running)
    )))
}
    ...
    match &target.watch {
        ...
        Some(w) => {
            porcelain::note(
                rt.term(),
                &format!(
                    "Stopped {} and recorded {}. View with `fjo times list {}`.",
                    label(w),
                    elapsed(w),
                    w.issue_index
                ),
...
async fn status(rt: &Runtime, api: &Api, globals: &GlobalOpts) -> Result<()> {
    ...
    let Some(w) = running(api).await? else {
        ...
        return porcelain::empty(
            ...
            rt.term(),
            machine.as_ref(),
            "No timer running. `fjo stopwatch start <issue>` starts one.",
        );
    };
...
/// (`[ "$(fjo stopwatch status | cut -f2)" -gt 3600 ]`) and cannot compare a Go duration.
pub(crate) fn render_status(term: &Term, w: &StopWatch) -> String {
    ...
    if term.tty {
        ...
        let _ = writeln!(o, "\nNot yet recorded. Save with `fjo stopwatch stop`.");
    } else {
...
/// `fjo stopwatch stop` work outside a checkout. Repository resolution is only attempted when
...
async fn target(
    ...
    match (explicit, watch) {
        ...
        (None, None) => Err(porcelain::usage(
            "no stopwatch is running. Start one with `fjo stopwatch start <issue>`.",
        )),
        (Some(index), w) => {
            let slug = rt.repo(globals)?.slug.clone();
...
mod tests {
    ...
    fn starting_a_second_timer_names_the_issue_already_being_timed() {
        ...
        assert!(msg.contains("fjo stopwatch stop"), "{msg}");
        assert!(msg.contains("fjo stopwatch cancel"), "{msg}");
    ...
    /// which is what makes `fjo stopwatch stop` work from a home directory — and the request it
