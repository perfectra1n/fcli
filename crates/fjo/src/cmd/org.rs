//! `fjo org` — organizations, and their membership.
//!
...
//! `fjo org member add` takes `--team`, resolves the team name to its id, and adds the user
...
//!
//! Nothing here resolves a repository, so `fjo org list` works anywhere — which is required by
