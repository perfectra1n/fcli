//! `fjo nodeinfo` — what kind of instance this is, **before you log in**.
//!
//! # The gap this fills
//!
//! Every other command in `fjo` assumes a configured host and a token. This one assumes neither,
...
//!
//! ```text
//! fjo --host codeberg.org nodeinfo
//! ```
//!
...
JSON fields follow each endpoint: NodeInfo uses camelCase; API settings use snake_case.
  ...
  fjo nodeinfo
  fjo nodeinfo --json software --jq .software.version
  fjo nodeinfo limits
  fjo --host codeberg.org nodeinfo";
...
async fn overview(api: &Api, globals: &GlobalOpts, term: &Term, host: &str) -> Result<()> {
    ...
    // than an optimisation: `fjo nodeinfo --jq '.bad['` is a typo in an argument, and it must be
...
pub(crate) fn render_overview(
    ...
    if !term.tty {
        ...
        // Four fields chosen so `fjo nodeinfo | cut -f2` is the version check a script wants.
        let _ = writeln!(
            o,
            "{}\t{}\t{}\t{}",
            name,
...
/// A line about what the reported software means for `fjo`.
///
/// The honest version of a version check: `fjo` is generated from Forgejo's specification, so a
...
fn flavour_note(name: &str) -> Option<String> {
    match name.trim().to_ascii_lowercase().as_str() {
        "forgejo" => None,
        "gitea" => Some(
            "This server runs Gitea. Forgejo-specific commands, including fjo quota and AGit, may be unavailable."
                .to_owned(),
        ),
        "" => Some(
        ...
        ErrorKind::NotAuthenticated { .. } | ErrorKind::TokenRejected { .. } => {
            Error::new(ErrorKind::Usage(
                "this instance requires a credential even for its public metadata, which means \
                 REQUIRE_SIGNIN_VIEW is on. Run `fjo auth login` first, or pass a token in \
                 $FORGEJO_TOKEN."
                    .to_owned(),
            ))
        }
...
mod tests {
    ...
    /// `fjo nodeinfo --json software` should not double its request count for it.
    #[tokio::test]
    ...
    fn the_json_output_uses_the_documents_own_field_names() {
        // camelCase here, because NodeInfo *is* camelCase — `fjo` never translates, so
    ...
    /// otherwise `fjo quota status` 404ing looks like a bug in fjo.
    #[test]
    fn a_non_forgejo_instance_is_named_and_forgejo_is_not_nagged_about() {
        ...
        assert!(gitea.contains("fjo quota"), "{gitea}");
