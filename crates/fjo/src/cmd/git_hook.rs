//! `fjo git-hook` — the server-side git hooks Forgejo runs on push.
//!
...
//! (`fjo raw repo list-git-hooks` and friends), so this group exists only for the three things
...
//! `fjo git-hook delete pre-receive` expecting the hook to stop existing has been misled by the
...
//!
//! ```text
//! fjo git-hook view pre-receive > pre-receive && chmod +x pre-receive
//! ```
//!
...
//! `fjo raw` kept working. Guessing wrong costs one 404 that names the hook.
...
requires an admin token. `disable` clears a hook's script; the hook remains listed.
  ...
  fjo git-hook list
  fjo git-hook view pre-receive > pre-receive.sh
  fjo git-hook edit pre-receive -F pre-receive.sh
  cat hook.sh | fjo git-hook edit update -F -
  fjo git-hook edit post-receive -e            # opens $EDITOR on what is there now
  fjo git-hook disable pre-receive --yes";
...
/// been switched off.
fn can_prompt() -> bool {
    terminals_attached() && std::env::var_os("FJO_PROMPT_DISABLED").is_none()
}
...
fn delete_is_not_what_it_sounds_like(hook: &str) -> forgejo_core::Error {
    support::usage(format!(
        "Git hooks cannot be deleted (fixed hooks: {}). Disable {hook:?} with `fjo git-hook disable {hook}`. This clears the script; the hook remains listed with active=false.",
        KNOWN_HOOKS.join(", ")
    ))
}
...
mod tests {
    ...
    fn delete_is_refused_with_the_truth_about_what_the_api_does() {
        ...
        assert!(text.contains("fjo git-hook disable pre-receive"), "{text}");
    ...
    /// newline with a space. `fjo git-hook view pre-receive > hook.sh` would then write one
    ...
    fn a_hook_name_is_sent_as_typed_so_a_newer_forgejo_stays_reachable() {
        ...
        assert!(e.to_string().contains("fjo git-hook disable proc-receive"), "{e}");
    }
    ...
    /// Bug this prevents: `fjo git-hook edit update` with no script spending a `GET` before
    ...
    /// Every subcommand has to be reachable and clap-consistent. `crates/fjo/tests/
