...
//!
//! So `fjo issue close 42` written the obvious way would close the issue and wipe its body,
//! and `fjo release edit v1 -n notes` would publish a draft. The bodies in this module are
...
impl Out {
    ...
    /// on **stdout**, with exit **0**, which is `fjo`'s deliberate divergence from `gh`.
...
impl Cx {
    ...
    /// For a command that works without one — `fjo label list --org acme`.
    ...
    pub fn browse(&self, url: &str) -> Result<()> {
        ...
        result.map_err(|e| {
            support::usage(format!("could not open a browser for {url}: {e}; open it yourself, or set `fjo config set browser <cmd>`"))
        })
    }
    ...
    pub fn edit_text(&self, initial: &str) -> Result<String> {
        ...
        if parts.is_empty() {
            return Err(support::usage(
                "no editor is configured; set $EDITOR or `fjo config set editor <cmd>`",
            ));
        }
...
fn prompt_disabled(env: &dyn forgejo_core::config::Env) -> bool {
    env.get("FJO_PROMPT_DISABLED").is_some_and(|v| !v.is_empty())
}
...
/// carrying meaning. Both reach `fjo issue view` and `fjo release view`, on a TTY only.
///
...
/// this, `fjo issue create -l bug` would be `fjo issue create -l 4`, which nobody can type
...
///
/// `fjo pr` reaches for this too, which is why it takes an `Api` and a slug rather than a
...
/// own labels, so `fjo pr create -l <an org label>` failed with "has no label called" for a
...
/// closing a milestone must not stop `fjo issue list -m 1.0` from working.
///
    ...
    match hits.len() {
        ...
        n => Err(support::usage(format!(
            "{} has {n} milestones titled {title:?} (ids {}); Forgejo does not require titles to \
             be unique, so rename or delete one — fjo will not guess which you meant",
            cx.repo()?,
...
mod tests {
    ...
    /// `fjo issue view 1 --json body --jq .body > file` and one doing `fjo issue view 1`
