//! `fjo completion` — a shell completion script.
//!
...
//!
//! So the generated script covers layer 1 (`fjo api`), all of layer 3, and the `raw` **group**
//! names, stopping there. `--help` says so, out loud: a completion that silently omits half the
//! tool teaches users the omitted half does not exist. `fjo raw search <words>` is the discovery
...
Completions include common commands, flags, and `fjo raw` group names.
Individual raw operations are not included. Find them with `fjo raw search <words>`
or `fjo raw <group> --help`.

  fjo completion bash > /etc/bash_completion.d/fjo
  fjo completion zsh  > \"${fpath[1]}/_fjo\"
  fjo completion fish > ~/.config/fish/completions/fjo.fish";
...
pub fn run(globals: &GlobalOpts, args: &Args) -> Result<()> {
    ...
    clap_complete::generate(clap_complete::Shell::from(args.shell), &mut cmd, "fjo", &mut out);
    out.flush()?;
    Ok(())
}
...
/// `fjo raw <group>` with no leaves under it.
///
/// The group names come from the generated table, so they cannot drift from what `fjo raw` will
...
/// Every top-level word `fjo` itself answers to, for `alias set`'s shadow check.
...
mod tests {
    ...
    fn every_shell_generates_something_that_mentions_the_groups() {
        for shell in <Shell as ValueEnum>::value_variants() {
            let mut buf = Vec::new();
            let mut cmd = root();
            clap_complete::generate(clap_complete::Shell::from(*shell), &mut cmd, "fjo", &mut buf);
    ...
    fn the_help_admits_what_it_does_not_cover() {
        ...
        assert!(LONG_HELP.contains("fjo raw search"), "{LONG_HELP}");
    }
}
