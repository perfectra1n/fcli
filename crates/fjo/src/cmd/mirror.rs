//! `fjo mirror` — push and pull mirrors.
//!
...
//! belongs to `fjo repo create --mirror-from`, which is another wave's file — so this group's
...
`fjo repo create <name> --mirror-from <url>`. Once created, use `mirror sync`
and `mirror status` to manage it.

  fjo repo create <name> --mirror-from <url>
  fjo mirror list
  fjo mirror add https://github.com/me/proj.git --username me --interval 8h
  fjo mirror add https://codeberg.org/me/proj.git --branch-filter 'main, release/*'
  fjo mirror sync
  fjo mirror status";
...
saved in shell history and Forgejo's stored remote. Use HTTPS for LFS support.
  ...
  fjo mirror add https://github.com/me/proj.git --username me
  fjo mirror add git@codeberg.org:me/proj.git --ssh
  fjo mirror add https://git.example.org/me/proj.git --branch-filter 'main, release/*'  --interval 30m --sync-on-commit")]
pub struct AddArgs {
...
pub struct DeleteArgs {
    /// The mirror's remote name, from the REMOTE column of `fjo mirror list`
...
async fn list(rt: &Runtime, api: &Api, globals: &GlobalOpts, slug: &RepoSlug) -> Result<()> {
    ...
    if mirrors.is_empty() {
        return porcelain::empty(
            ...
            machine.as_ref(),
            &format!(
                "{slug} has no push mirrors. Add one with `fjo mirror add <address>`. For a new pull mirror, use `fjo repo create --mirror-from`."
            ),
        );
    }
    ...
    if mirrors.iter().any(|m| !m.last_error.trim().is_empty()) {
        porcelain::note(
            ...
            "A mirror with an ERROR is not retrying on its own schedule until the cause is fixed; \
             `fjo mirror sync --push` retries now.",
        );
    }
    Ok(())
}
...
async fn sync(rt: &Runtime, api: &Api, slug: &RepoSlug, args: &SyncArgs) -> Result<()> {
    ...
    if !do_pull && !do_push {
        return Err(porcelain::usage(format!(
            "{slug} has no mirrors to sync. Add a push mirror with `fjo mirror add <address>`. Pull mirrors must be created with `fjo repo create --mirror-from`."
        )));
    }
    ...
    if do_push {
        ...
        porcelain::note(
            rt.term(),
            &format!(
                "Queued a push to all mirrors of {slug}. Check results with `fjo mirror list`."
            ),
        );
    }
    Ok(())
...
pub(crate) fn render_status(
    ...
    let _ = writeln!(o);
    if mirrors.is_empty() {
        let _ = writeln!(o, "push mirrors none  (`fjo mirror add <address>`)");
        return o;
    }
    ...
    match &*e.kind {
        ErrorKind::RouteNotFound { .. } => Error::new(ErrorKind::Usage(
            "this instance did not answer the mirror endpoint. Mirroring can be switched off \
             ...
             ALLOW_PUSH_MIRRORS = false) — ask an administrator, or check `fjo nodeinfo`."
                .to_owned(),
        )),
        _ => e,
    }
...
fn explain_named(e: Error, remote: &str) -> Error {
    match &*e.kind {
        ErrorKind::ResourceNotFound { .. } => Error::new(ErrorKind::Usage(format!(
            "no push mirror named {remote:?}. Use the generated name from the REMOTE column of `fjo mirror list`."
        ))),
        _ => explain(e),
    }
}
