...
//! [`crate::output`].
//!
//! `fjo-raw` deliberately does no I/O — it turns [`clap::ArgMatches`] into a described request
...
/// The layer-2 root: `fjo` with the global flags and **only** the named subtree.
///
...
pub fn root(group: Option<&str>, leaf: Option<&str>) -> clap::Command {
    let raw =
        fjo_raw::raw_command(forgejo_client::meta::OPS, forgejo_client::meta::GROUPS, group, leaf);
    let taken = crate::global::taken(&raw);
    crate::global::augment(
        clap::Command::new("fjo")
            .about(crate::ABOUT)
...
pub fn run(globals: &GlobalOpts, op: &'static OpMeta, m: &ArgMatches) -> Result<()> {
    ...
    if m.try_get_one::<bool>(fjo_raw::build::ID_DRY_RUN).ok().flatten().copied().unwrap_or(false) {
        let plan =
            fjo_raw::bind(op, m, best_effort_slug(globals, op).as_ref(), &mut std::io::stdin())?;
        let mut out = std::io::stdout().lock();
    ...
    crate::runtime::block_on(async move {
        ...
        let plan = fjo_raw::bind(op, m, slug.as_ref(), &mut std::io::stdin())?;
    ...
    match plan.op.produces {
        Produces::Json | Produces::LdJson => {
            ...
            if paginate_wanted && plan.op.pagination == meta_types::Pagination::Paged {
                ...
                // `--json`/`--jq` should see it as one list the way `fjo pr list` will.
                let merged = paginate::flatten(walked.pages);
                return render(rt, globals, &pipeline, merged);
            }
            if paginate_wanted {
                return Err(usage(format!(
                    "fjo raw {} {} does not support pagination; remove --paginate",
        ...
        Produces::Text | Produces::Html => {
            if pipeline.is_explicit() {
                return Err(usage(format!(
                    "fjo raw {} {} answers with {}, not JSON, so --json/--jq/--template have \
                     nothing to work on",
                    plan.op.group,
                    plan.op.command,
...
fn resolve_fields(op: &'static OpMeta, raw: &str) -> Result<Selection> {
    ...
    if fields.is_empty() {
        return Err(usage(format!(
            "fjo raw {} {} has no selectable fields: it answers with {}, not a typed object.\n\
...
/// structurally identical and deliberately distinct types: `fjo::output` is written against no
...
mod tests {
    ...
    fn plan_of(group: &str, command: &str, words: &[&str]) -> PlannedRequest {
        ...
        let cmd = fjo_raw::build(OPS, GROUPS, Some(group), Some(command));
        let mut argv: Vec<String> = vec!["fjo".into(), "raw".into(), group.into(), command.into()];
        ...
        let leaf = m
            ...
            .expect("leaf matches");
        fjo_raw::bind(op, leaf, None, &mut std::io::empty()).unwrap()
    }
