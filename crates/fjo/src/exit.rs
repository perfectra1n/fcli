...
//!    was added.
//! 2. **A broken pipe is success.** `fjo pr list | head -1` closes stdout under us on
...
/// they go, and `fjo … | jq` should still get a coloured error.
...
fn notes(color: render::Color) {
    ...
    let _ = writeln!(err, "      Silence this with FJO_NO_COMPAT_NOTES=1.");
}
...
mod tests {
    ...
    /// Bug this prevents: `fjo … | head -1` exiting non-zero, which turns every such pipeline
    ...
    fn a_keyring_warning_keeps_its_advice() {
        ...
        assert!(rendered.contains("FJO_CREDENTIAL_STORE"), "{rendered}");
    }
}
