...
//!
//! These deliberately go through `fjo::output`'s exported API only, so they fail if a type the
...
use fjo::output::{Dest, FieldKind, FieldSpec, Filter, Pipeline, Selection, Table, Template, Term};
...
/// Bug this prevents: the two rendering paths drifting apart, so `fjo pr list | cut -f2`
...
fn wide_characters_do_not_break_column_alignment() {
    ...
    let column_starts: Vec<usize> = out
        ...
        .map(|line| {
            ...
            fjo::output::display_width(before) + 2
        })
        .collect();
...
/// which is what lets `fjo pr list --json` answer without auth or connectivity.
#[test]
fn field_discovery_is_offline_and_exits_successfully() {
    assert_eq!(fjo::output::project::resolve("", FIELDS).unwrap(), Selection::Discover);

    let mut piped = Vec::new();
    fjo::output::project::write_field_list(FIELDS, &Term::piped(), &mut piped).unwrap();
    assert_eq!(String::from_utf8(piped).unwrap(), "number\ntitle\nstate\nhtml_url\n");

    let mut tty = Vec::new();
    fjo::output::project::write_field_list(FIELDS, &Term::tty(100), &mut tty).unwrap();
...
fn binary_output_to_a_terminal_is_refused() {
    let err =
        fjo::output::guard_binary("application/octet-stream", &Term::tty(80), &Dest::Stdout, false)
            .unwrap_err();
    assert_eq!(err.exit_code(), 2);
    assert!(
        fjo::output::guard_binary("application/octet-stream", &Term::tty(80), &Dest::Stdout, true)
            .is_ok()
    );
}
