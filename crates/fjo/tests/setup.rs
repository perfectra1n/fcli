...
/// keyring — into a test.
///
/// `FJO_CREDENTIAL_STORE=file` is not laziness: without it these tests would probe the developer's
/// login keyring, which is both slow and rude.
fn cmd(dir: &Path) -> Command {
    let mut c = Command::cargo_bin("fjo").expect("the fjo binary is built for its own tests");
    c.env("FJO_CONFIG_DIR", dir)
...
fn auth_status_never_prints_the_token_on_either_stream() {
    ...
    let out = tty(dir.path()).args(["auth", "status"]).output().expect("fjo runs");
...
fn auth_status_json_has_no_token_field_at_all() {
    ...
    let out = cmd(dir.path()).args(["auth", "status", "--json"]).output().expect("fjo runs");
...
fn auth_status_reports_the_first_host_in_the_file_when_several_fail() {
    for (broken_first, expected) in [(false, 4), (true, 6)] {
        ...
        let out = cmd(dir.path()).args(["auth", "status"]).output().expect("fjo runs");
        let stdout = String::from_utf8_lossy(&out.stdout);
...
/// piped — no trailing newline, no banner — or `$(fjo auth token)` and `curl --config -` disagree.
#[test]
...
/// `setup-git` must name *this* executable, not the bare word `fjo`, and its `--dry-run` has to
...
fn config_set_get_honours_per_host_overrides() {
    ...
    let value = |args: &[&str]| -> String {
        let out = cmd(dir.path()).args(args).output().expect("fjo runs");
...
/// The end-to-end claim this whole feature makes: `fjo alias set X '<command>'` then `fjo X`.
///
...
fn an_alias_is_expanded_before_the_command_line_is_parsed() {
    ...
    cmd(dir.path()).arg("shell").assert().success().stdout(contains("complete -c fjo"));
...
fn an_alias_that_shadows_a_real_command_is_refused_by_name() {
    ...
    cmd(dir.path())
        ...
        .assert()
        .code(2)
        .stderr(contains("already a real fjo command"))
        .stderr(contains("fjo pr --help"));
...
/// `fjo co 42 --force` would silently drop `--force`.
#[test]
fn placeholders_consume_positionally_and_the_rest_is_appended() {
    ...
    cmd(dir.path()).args(["shell", "fish"]).assert().success().stdout(contains("complete -c fjo"));
...
fn alias_import_skips_what_it_cannot_use_and_keeps_the_rest() {
    ...
    let out = cmd(dir.path()).args(["alias", "import"]).arg(&file).output().expect("fjo runs");
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    assert!(
    ...
    // fjo's documented divergence from `gh` (docs/output.md, divergence 2).
...
fn completion_covers_layers_one_and_three_and_admits_what_it_omits() {
    ...
    for shell in ["bash", "zsh", "fish", "powershell", "elvish"] {
        let out = cmd(dir.path()).args(["completion", shell]).output().expect("fjo runs");
    ...
    cmd(dir.path())
        ...
        .stdout(contains("fjo raw search"));
}
...
fn human_output_goldens() {
    ...
    for args in [
        ...
        let out = tty(dir.path()).args(args).output().expect("fjo runs");
        report.push_str(&format!("== fjo {}\n", args.join(" ")));
