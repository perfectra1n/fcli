...
/// `fcli issue pin N --position 0` used to send `POST …/pin`, pin the issue, and only then exit 2
...
fn issue_pin_refuses_position_zero_without_pinning_anything() {
    ...
    let run = inst.fcli(["issue", "pin", "1", "--position", "0", "-R", &repo.slug()]);
    run.assert_code(2, "fcli issue pin --position 0");
...
fn notification_mark_read_marks_every_thread_it_listed() {
    ...
    run.assert_ok("fcli notification list --mark-read");
    assert_eq!(
...
fn workflow_list_reads_all_three_directories_and_keeps_bodies_with_their_paths() {
    ...
    let run = inst.fcli(["workflow", "list", "-R", &repo.slug(), "--json", "path,workflow_name"]);
    run.assert_ok("fcli workflow list");
    let rows = run.json();
...
fn workflow_list_tolerates_the_two_directories_that_are_not_there() {
    ...
    let run = inst.fcli(["workflow", "list", "-R", &repo.slug(), "--json", "path,workflow_name"]);
    run.assert_ok("fcli workflow list where two of the three directories do not exist");
    let rows = run.json();
...
fn label_list_include_org_survives_an_owner_that_is_not_an_organization() {
    ...
    let run = inst.fcli(["label", "list", "-R", &repo.slug(), "--include-org", "--json", "name"]);
    run.assert_ok("fcli label list --include-org on a user-owned repository");
...
fn issue_view_with_comments_prints_all_of_them_in_order() {
    ...
    let run = inst.fcli(["issue", "view", "1", "-c", "-R", &repo.slug()]);
    run.assert_ok("fcli issue view -c");
...
fn repo_view_prints_the_readme_it_fetched_concurrently() {
    ...
    let run = inst.fcli_env(
        std::path::Path::new("."),
        &[("FCLI_FORCE_TTY", "100")],
        ["repo", "view", &repo.slug()],
    );
    run.assert_ok("fcli repo view");
    run.assert_says("README_MARKER_FOR_CONCURRENT_VIEW");

    let run = inst.fcli(["repo", "view", &repo.slug(), "--json", "full_name,private"]);
    run.assert_ok("fcli repo view --json");
    assert_eq!(
...
fn issue_depends_list_shows_both_directions() {
    ...
    inst.fcli(["issue", "depends", "add", "1", "--blocked-by", "2", "-R", &repo.slug()])
        .assert_ok("fcli issue depends add --blocked-by");
    inst.fcli(["issue", "depends", "add", "1", "--blocks", "3", "-R", &repo.slug()])
        .assert_ok("fcli issue depends add --blocks");

    let run = inst.fcli(["issue", "depends", "list", "1", "-R", &repo.slug()]);
    run.assert_ok("fcli issue depends list");
    let blocked_by = run
        .stdout
        .lines()
...
fn pr_machine_output_is_complete_without_the_calls_it_stopped_making() {
    ...
    let run = inst.fcli(["pr", "view", "1", "-c", "-R", &repo.slug(), "--json", "number,title"]);
    run.assert_ok("fcli pr view -c --json");
    let pr = run.json();
    assert_eq!(pr["number"].as_u64(), Some(1), "{}", run.stdout);
    assert_eq!(pr["title"].as_str(), Some("machine readable"), "{}", run.stdout);

    let run =
        inst.fcli(["pr", "status", "1", "-R", &repo.slug(), "--json", "number,title,mergeable"]);
    run.assert_ok("fcli pr status --json");
    let status = run.json();
    ...
    let run = inst.fcli(["pr", "view", "1", "-c", "-R", &repo.slug()]);
    run.assert_ok("fcli pr view -c");
...
fn block_list_rows_follow_the_api_listing_order() {
    ...
    for login in &logins {
        let (code, body) = inst.api(
            ...
            "admin/users",
            Some(&format!(
                r#"{{"username":"{login}","email":"{login}@example.invalid","password":"fcli-itest-blocked-1","must_change_password":false}}"#
            )),
        );
        assert!(
        ...
        inst.fcli(["block", "add", login]).assert_ok("fcli block add");
    }
    ...
    let run = inst.fcli(["block", "list", "--limit", "100"]);
    run.assert_ok("fcli block list");
    ...
    for login in &logins {
        let _ = inst.fcli(["block", "remove", login]);
    }
}
...
fn times_list_all_still_filters_by_the_login_it_resolved() {
    ...
    let run =
        inst.fcli(["times", "list", "--all", "--mine", "--limit", "100", "--json", "id,user_name"]);
    run.assert_ok("fcli times list --all --mine");
    let rows = run.json();
...
/// `fcli nodeinfo --jq <broken>` must fail on the expression before it sends anything. The
...
fn a_broken_jq_expression_beats_the_request_it_would_have_filtered() {
    ...
    let run = inst.fcli(["nodeinfo", "--jq", ".bad["]);
    run.assert_code(2, "fcli nodeinfo --jq with a broken expression");
