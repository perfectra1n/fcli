...
//!
//! `fcli pr create --agit --topic <t>` pushes to `refs/for/<base>/<topic>`, and Forgejo turns
...
use fcli_itest::{TestRepo, git, instance_or_skip};

struct Scratch(PathBuf);
impl Scratch {
    fn new(tag: &str) -> Self {
        let d = std::env::temp_dir().join(format!("fcli-itest-agit-{}-{tag}", std::process::id()));
...
///
/// `fcli pr create` opened with an unconditional `GET /repos/{owner}/{repo}`, whose only consumer
...
fn a_dry_run_with_an_explicit_base_asks_the_server_nothing() {
    ...
    let run = inst.fcli_env(
        scratch.path(),
        &[("FCLI_TOKEN", "definitely-not-a-valid-token")],
        [
            "pr",
            "create",
            "--agit",
