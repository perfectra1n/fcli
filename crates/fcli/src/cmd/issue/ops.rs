...
//!   as issues, so without it `fcli issue list` shows pull requests — and then
//!   `fcli issue close` on one of them mangles a pull request.
...
async fn create(cx: &Cx, a: &CreateArgs) -> Result<()> {
    ...
    if let Some(path) = &a.recover {
        ...
        let saved: Draft = serde_json::from_str(&text).map_err(|e| {
            support::usage(format!("--recover {}: not a draft fcli wrote ({e})", path.display()))
        })?;
        draft = Draft {
    ...
    let issue = match cx.api.issue().create_issue(cx.owner()?, cx.name()?, &body).await {
        ...
        Err(e) => {
            ...
            if let Some(path) = save_draft(&draft) {
                eprintln!(
                    "the issue was not created; your draft is at {} — retry with `fcli issue \
                     create --recover {}`",
                    path.display(),
                    path.display()
                );
...
fn save_draft(draft: &Draft) -> Option<std::path::PathBuf> {
    let path = std::env::temp_dir().join(format!("fcli-issue-draft-{}.json", std::process::id()));
...
async fn list(cx: &Cx, globals: &GlobalOpts, a: &ListArgs) -> Result<()> {
    ...
    // arrives in `fcli issue list` is one `fcli issue close` away from real damage.
    let mut query =
...
fn not_an_issue(index: IssueIndex) -> Error {
    Error::new(ErrorKind::Usage(format!(
        "#{index} is a pull request, not an issue; use `fcli pr` for it"
    )))
}
...
mod tests {
    ...
    ///
    /// `http` is not a direct dependency of `fcli` — `forgejo_core::http` re-exports what the
    ...
    fn issue_42() -> String {
        ...
        "body": "# Steps\n\n- one\n- two\n",
        "html_url": "https://git.example.org/perf3ct/fcli/issues/42",
    ...
    async fn add_label_adds_and_never_replaces() {
        let fake = Arc::new(
            FakeTransport::new()
                .on(
                    method!("POST"),
                    "/api/v1/repos/perf3ct/fcli/issues/42/labels",
                    Canned::json(200, "[]"),
                )
                .on(
                    method!("GET"),
        ...
        let posts = fake.calls_to(&method!("POST"), "/api/v1/repos/perf3ct/fcli/issues/42/labels");
    ...
    /// Bug this prevents: `fcli issue list` showing pull requests, because
    ...
    async fn list_asks_for_issues_only() {
        ...
        let call = fake
            .calls()
            .into_iter()
            .find(|c| c.path == "/api/v1/repos/perf3ct/fcli/issues")
    ...
    async fn a_truncated_list_asks_the_collection_how_many_there_were() {
        ...
        let out = text(&buf);
        assert!(
            out.starts_with("Showing 1 of 43 open issues in perf3ct/fcli\n"),
        ...
        let probe = fake
            .calls()
            .into_iter()
            .rfind(|c| c.path == "/api/v1/repos/perf3ct/fcli/issues")
    ...
    async fn a_complete_list_costs_no_extra_request() {
        ...
        assert!(text(&buf).starts_with("Showing 1 open issue in perf3ct/fcli\n"), "{}", text(&buf));
        // Two: the one page of issues, and the `[]` that ends the walk. No third.
        let hits = fake
            .calls()
            .into_iter()
            .filter(|c| c.path == "/api/v1/repos/perf3ct/fcli/issues")
            .count();
    ...
    async fn dependency_directions_hit_their_own_endpoints() {
        ...
        assert_eq!(calls[0].path, "/api/v1/repos/perf3ct/fcli/issues/42/dependencies");
    ...
    async fn editing_a_comment_uses_the_comment_id_route() {
        let fake = Arc::new(FakeTransport::new().on(
            method!("PATCH"),
            "/api/v1/repos/perf3ct/fcli/issues/comments/555",
            Canned::json(
                200,
        ...
        assert_eq!(fake.calls()[0].path, "/api/v1/repos/perf3ct/fcli/issues/comments/555");
    ...
    async fn an_empty_list_is_success_with_an_empty_table() {
        let fake = Arc::new(
            FakeTransport::new()
                ...
                .on(method!("GET"), "/api/v1/repos/perf3ct/fcli/issues", Canned::json(200, "[]")),
        );
        let buf = Arc::new(Mutex::new(Vec::new()));
        let globals = GlobalOpts::default();
        let cx = cx(fake, &buf, &globals, crate::output::Term::piped());
