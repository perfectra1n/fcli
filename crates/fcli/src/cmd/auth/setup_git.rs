//! `fcli auth setup-git` — teach `git` to authenticate with the token `fcli` already has.
//!
//! Without this, a user who has just run `fcli auth login` still gets a password prompt from
...
const LONG_HELP: &str = "\
Configure Git to authenticate over HTTPS using your saved fcli token.
...
appear in .git/config and git remote -v output.
  ...
  fcli auth setup-git --host git.example.org
  fcli auth setup-git --host git.example.org --dry-run";
...
pub fn run(globals: &GlobalOpts, args: &Args) -> Result<()> {
    ...
    // only sends the path to a helper when asked to. Without this, two Forgejo instances at
    ...
    writeln!(out, "git will now authenticate to {key} with your fcli token")?;
    writeln!(out, "Helper: {helper}")?;
    out.flush()?;
    Ok(())
}

/// The value git stores, e.g. `!/usr/local/bin/fcli auth git-credential`.
///
...
/// the bare word `fcli`, so a user running `./target/debug/fcli` does not end up with a git
/// configuration pointing at some other `fcli` on their `$PATH`.
fn helper_command() -> Result<String> {
    let exe = std::env::current_exe().map_err(|e| {
        support::usage(format!(
            "cannot find the fcli executable path for the Git credential helper: {e}"
        ))
    })?;
...
/// `git config --global` has no equivalent on [`forgejo_core::context::GitCtx`], which exposes
...
/// `docs/porcelain-conventions.md` reserves to `forgejo_core::context`.
...
mod tests {
    ...
    /// Bug this prevents: a helper entry that names the bare word `fcli`, so a developer testing
    /// `./target/debug/fcli auth setup-git` silently configures git to use whatever other `fcli`
    ...
    fn a_path_with_a_space_is_quoted() {
        ...
        assert_eq!(quote("/a b/fcli"), "!'/a b/fcli' auth git-credential");
        assert_eq!(quote("/a/fcli"), "!/a/fcli auth git-credential");
        assert_eq!(quote("/it's/fcli"), r"!'/it'\''s/fcli' auth git-credential");
    }
}
