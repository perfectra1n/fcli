//! `fcli auth token` — the deliberate, single exit through which a token becomes plaintext.
//!
//! `auth status` will not print a token under any flag. This command will, because a script
//! genuinely needs one (`curl -H "Authorization: token $(fcli auth token)"`, a `docker login`,
...
or recordings. Piped output has no trailing newline.
  ...
  curl -H \"Authorization: token $(fcli auth token --host git.example.org)\" ...";
...
pub fn run(globals: &GlobalOpts, _args: &Args) -> Result<()> {
    ...
    support::note(
        &term,
        "warning: this token is now in your terminal's scrollback; `fcli auth token | pbcopy` \
         or a pipe keeps it out of your history",
    );

    let mut out = support::writer(globals)?;
    // A newline for a human, none for a pipe. `$(fcli auth token)` strips a trailing newline
