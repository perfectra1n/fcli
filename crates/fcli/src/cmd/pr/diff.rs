//! `fcli pr diff` — the diff, with escape sequences defanged.
//!
...
//! this reason and it is a security convention rather than a formatting preference, so `fcli`
...
//! name, which clap answers with a panic. So `fcli pr diff --color never` works, and there is no
...
Color is controlled by the global --color option.
  ...
  fcli pr diff
  fcli pr diff 42 --name-only
  fcli pr diff --patch > change.patch
  fcli pr diff --color always | less -R")]
pub struct Args {
...
pub fn run(globals: &GlobalOpts, args: &Args) -> Result<()> {
    crate::runtime::block_on(async move {
        ...
        if args.name_only {
            let query = forgejo_client::query::RepoGetPullRequestFilesQuery::default();
        ...
        let mut query = forgejo_client::query::RepoDownloadPullDiffOrPatchQuery::default();
        if args.binary {
...
mod tests {
    ...
    fn name_only_and_patch_are_mutually_exclusive() {
        ...
        assert!(
            <Harness as clap::Parser>::try_parse_from(["fcli", "--name-only", "--patch"]).is_err()
        );
    }
}
