//! `fcli pr checks` and `fcli pr status`.
//!
...
//!
//! ```sh
//! fcli pr checks; case $? in 0) merge;; 8) sleep 60;; *) investigate;; esac
//! ```
//!
//! `fcli` matches it, through the taxonomy rather than around it:
...
//! never came, and `RunFailed` points at `fcli run view <run>`, which is wrong for any check
...
Exit codes: 0 for passed or no checks, 1 for failed checks, 8 for pending checks.
  ...
  fcli pr checks
  fcli pr checks 42
  fcli pr checks --json state,statuses
  fcli pr checks; case $? in 0) echo green;; 8) echo waiting;; *) echo red;; esac")]
pub struct Args {
