//! `fcli pr review` — approve, comment, or request changes.
//!
...
requests require a body. Supply one with -b, -F, or the interactive prompt.
  ...
  fcli pr review --approve
  fcli pr review 42 --request-changes -b 'the lexer still mishandles CRLF'
  fcli pr review --comment -F notes.md
  fcli pr review --approve -F -")]
pub struct Args {
...
///
/// No default. A `fcli pr review` that quietly approved would be the worst possible default, and one
...
mod tests {
    ...
    fn each_flag_maps_to_the_apis_event_value() {
        assert_eq!(event_of(&args(&["fcli", "-a"])).unwrap().as_str(), "APPROVED");
        assert_eq!(event_of(&args(&["fcli", "-c"])).unwrap().as_str(), "COMMENT");
        assert_eq!(event_of(&args(&["fcli", "-r"])).unwrap().as_str(), "REQUEST_CHANGES");
    }
    ...
    fn a_review_with_no_verdict_is_a_usage_error() {
        let e = event_of(&args(&["fcli"])).unwrap_err();
