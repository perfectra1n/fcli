//! `fcli workflow` — Actions workflow files: list, inspect, dispatch, enable, disable.
//!
...
//! either. `fcli workflow run ci.yml` and `fcli workflow run .forgejo/workflows/ci.yml` both
//! work: the path form is resolved against the discovered files and only the base name is sent.
//!
//! # `-f`, `-F`, and where `gh`'s `--json` went
//!
//! Inputs use `crate::api::fields`, the *same* parser as `fcli api`, so `-f` (always a string)
...
//! spelled like `fcli api --input`, because `--json` is a global output selector in `fcli` and
...
pub struct ListArgs {
    ...
    /// Only workflows that can be started with `fcli workflow run`
    #[arg(long)]
    ...
    let mut rows = vec![
        ...
        (
            ...
            if parsed.dispatchable() {
                format!("yes: `fcli workflow run {}`", entry.name)
            } else {
...
async fn dispatch(rt: &Runtime, globals: &GlobalOpts, api: &Api, args: &RunArgs) -> Result<()> {
    ...
    emit::detail(
        ...
        vec![
            ...
            ("next".to_owned(), format!("fcli run watch {}", started.id)),
        ],
    )
}
    ...
    let Some(declared) = &parsed.dispatch else {
        return Err(support::usage(format!(
            "{filename} declares no workflow_dispatch trigger, so it cannot be started from the \
             API; add `on: workflow_dispatch:` to the file, or run `fcli workflow list \
...
/// base64, and `fcli` has no base64 decoder (nor a reason to grow one for this).
...
",
    ...
    /// `fcli api`'s field convention: `-f`/`-F` override the JSON object key by key.
    #[test]
