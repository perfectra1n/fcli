//! `fcli alias` — shortcuts for commands you run often, plus the expansion that makes them work.
//!
//! # Expansion happens before parsing
//!
//! `fcli prs` cannot be a clap subcommand: clap would have to be told about it, and the alias
//! table is not known until `config.toml` has been read. So [`expand`] rewrites `argv` *before*
//! either parse phase, exactly as `gh` does. `fcli alias set prs 'pr list --json number,title'`
//! then makes `fcli prs` become `fcli pr list --json number,title`.
//!
...
//!   with. Without that, `fcli alias set pr 'pr list'` would make `fcli pr create` unreachable and
...
//!   `alias set co 'pr checkout $1'` makes `fcli co 42 --force` become
//!   `fcli pr checkout 42 --force`, and `fcli co` fails naming how many arguments `co` needs
...
///
/// `gh`'s names are `fcli`'s names, so these do **not** appear in `--help`, in completions, or in
/// `fcli alias list`. They exist for one reason: somebody with `tea` in their fingers types
/// `fcli pull ls` and should land on `fcli pr ls` rather than on "unrecognized subcommand". Two of
...
/// `fcli alias set pull 'pr list --state all'` does exactly what it says.
...
whoami -> auth status. They are hidden from `alias list`; user aliases can replace them.
  ...
  fcli alias set prs 'pr list --json number,title'
  fcli prs                                          # runs the above
  fcli alias set co 'pr checkout $1'
  fcli co 42 --force                                # -> pr checkout 42 --force
  fcli alias list
  fcli alias delete co
  fcli alias import my-aliases.toml
  fcli alias import -                               # from stdin";
...
/// A usable alias name that does not collide with anything `fcli` already answers to.
///
...
fn check_name(name: &str) -> Result<()> {
    ...
    if super::completion::command_names().iter().any(|c| c == name) {
        return Err(support::usage(format!(
            "{name:?} is already a real fcli command, so an alias by that name could never run; \
             `fcli {name} --help` shows what it does. Pick another name."
        )));
    }
    Ok(())
}

/// The expansion must be non-empty, must start with something `fcli` can actually run, and must
/// not close a cycle.
fn check_expansion(name: &str, expansion: &str, existing: &BTreeMap<String, String>) -> Result<()> {
    ...
    if head.starts_with('-') {
        return Err(support::usage(format!(
            "the expansion for {name:?} starts with the flag {head:?}; it has to start with a \
             command, because that is the word fcli will look up"
        )));
    }
    ...
    if !known_command && !known_alias {
        return Err(support::usage(format!(
            "{head:?} is not an fcli command or an existing alias, so {name:?} could never run; \
             `fcli --help` lists the commands and `fcli raw search {head}` searches the API"
        )));
    }
    ...
    if let Some(chain) = find_cycle(name, &table) {
        return Err(support::usage(format!(
            "that would make an alias loop: {}. fcli would expand it forever, so it is refused.",
            chain.join(" -> ")
        )));
    }
    Ok(())
...
fn list(globals: &GlobalOpts, _args: &ListArgs) -> Result<()> {
    ...
    // Emptiness is not an error, and the note goes to stderr so `fcli alias list | wc -l` stays
    // honest.
    if aliases.is_empty() {
        support::note(
            &term,
            "no aliases configured. Add one with `fcli alias set <name> '<command>'`.",
        );
    }
...
fn delete(_globals: &GlobalOpts, args: &DeleteArgs) -> Result<()> {
    ...
    let Some(expansion) = config.alias(name) else {
        return Err(support::usage(format!(
            "no alias {name:?}; `fcli alias list` shows the ones you have"
        )));
    };
...
///
/// `toml` is deliberately not a dependency of the `fcli` binary: all configuration parsing lives in
...
/// the command word.
///
/// Mirrors `fcli_raw::build`'s own list for the same reason it exists there: a wrong guess costs a
...
/// Resolve `name` plus `args` into the words `fcli` should actually parse.
fn expand_words(
    name: &str,
    args: &[String],
    ...
    for _ in 0..MAX_DEPTH {
        if seen.contains(&head) {
            ...
            return Err(support::usage(format!(
                "the alias {name:?} expands in a loop: {}. Fix it with `fcli alias delete \
...
/// useful one: `alias set co 'pr checkout $1'` still lets `fcli co 42 --force` pass `--force`
/// through.
...
/// command line for `fcli`, and running it through a shell would make `alias set x 'repo view;
...
mod tests {
    ...
    /// looping forever, so `fcli x` hangs with no output and no clue.
    #[test]
    ...
    fn an_alias_may_not_shadow_a_real_command() {
        ...
        assert!(e.to_string().contains("already a real fcli command"), "{e}");
    ...
    fn an_expansion_must_start_with_something_that_exists() {
        ...
        assert!(e.to_string().contains("not an fcli command"), "{e}");
    ...
    /// Bug this prevents: taking `argv[1]` blindly, so `fcli --debug prs` does not expand and the
    ...
    fn the_command_word_is_found_past_leading_global_flags() {
        ...
        assert_eq!(command_word_index(&argv(&["fcli", "prs"])), Some(1));
        assert_eq!(command_word_index(&argv(&["fcli", "--debug", "prs"])), Some(2));
        assert_eq!(command_word_index(&argv(&["fcli", "--host", "h", "prs"])), Some(3));
        assert_eq!(command_word_index(&argv(&["fcli", "--host=h", "prs"])), Some(2));
        assert_eq!(command_word_index(&argv(&["fcli", "--help"])), None);
    }
    ...
    /// command line through — `fcli pull ls --state all` is the whole point, not bare `fcli pull`.
    #[test]
    ...
    /// `fcli alias list` shows one thing and `fcli whoami` does another.
    #[test]
    ...
    /// Every built-in has to expand to something `fcli` can actually run, or it is a worse
