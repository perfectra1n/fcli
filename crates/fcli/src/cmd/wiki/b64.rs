...
//!
//! `fcli` has no base64 dependency, and `forgejo-core` keeps its own fourteen-line encoder
...
//! `sidebar` and `footer`, which Forgejo also encodes — the field names carry no hint. A
//! future or patched instance that sends one of them as plain text must not make
//! `fcli wiki view` fail; showing the text is strictly better than showing an error, and a page
...
mod tests {
    ...
    /// `content_base64` is encoded but says nothing about `sidebar` and `footer`, which Forgejo
    /// also encodes. If an instance ever sends one of those as plain text, `fcli wiki view` must
