//! `fcli repo set-default` — write down which remote is *the* repository.
//!
//! Repository resolution scores remote names (`upstream` 3, `forgejo`/`codeberg` 2, `origin` 1) and
...
//!
//! What it writes is git config `remote.<name>.fcli-resolved`, namespaced per remote exactly as
...
//! | `NONE` | stop asking: this checkout has no Forgejo repository |
//!
...
repository. The picker also lets you disable automatic selection for this checkout.
  ...
  fcli repo set-default --view
  fcli repo set-default forgejo/forgejo
  fcli repo set-default --unset")]
pub struct Args {
    /// The repository to record. It should be one this checkout has a remote for
    #[arg(value_name = "REPOSITORY")]
    pub repo: Option<String>,

    /// Print what fcli currently resolves, and why
...
///
/// The `source` line is the point. "fcli is talking to them/proj" invites the question "why?", and
...
fn candidates(rt: &Runtime) -> Result<Vec<Candidate>> {
    use forgejo_core::context::remote_url;
...
///
/// Without it, a user in a checkout with no Forgejo remote is asked the same unanswerable question
...
fn pick(rt: &Runtime, candidates: &[Candidate]) -> Result<()> {
    if !support::can_prompt(rt) {
        return Err(Error::new(ErrorKind::Usage(
            "there is no terminal to show a picker on; name the repository \
             (`fcli repo set-default owner/name`), or use --view / --unset"
                .to_owned(),
        )));
    }
    ...
    let chosen =
        support::interact::select("Which repository should fcli commands act on?", &labels)?;
    if chosen == candidates.len() {
        ...
        support::note(
            rt.term(),
            &format!(
                "recorded {RESOLVED_NONE} on remote {}; fcli will stop guessing here",
                candidates[0].remote
            ),
        );
        return Ok(());
    ...
    match &candidates[chosen].slug {
        ...
        None => Err(Error::new(ErrorKind::Usage(format!(
            "remote {} ({}) does not identify a repository on {}. Run `fcli repo set-default owner/name`.",
