//! `fcli repo view` — the repository, and its README when a human is reading.
//!
...
//! **only on a terminal**: `fcli repo view | …` is a data pipeline, and forty kilobytes of prose in
...
Use --json to select repository fields.
  ...
  fcli repo view
  fcli repo view forgejo/forgejo
  fcli repo view -w
  fcli repo view --json full_name,stars_count,default_branch")]
pub struct Args {
...
/// those is a reason for `fcli repo view` to fail, because the repository *was* found and its
...
pub(crate) fn render(repo: &Repository, readme: Option<&str>, term: &Term) -> String {
    ...
    // A small table rather than hand-aligned text, so it lines up with every other `fcli` view
    ...
    match readme {
        ...
        // Saying so is better than an unexplained gap: "is the README missing, or did fcli not
...
/// output is this tool's machine channel: `fcli repo view > README.md` must write the README the
...
mod tests {
    ...
    /// cannot tell "there is no README" from "fcli failed to fetch it".
    #[test]
    ...
    /// Bug this prevents: `fcli repo view > README.md` writing a corrupted README, and
    /// `fcli repo view | grep '^## Install'` matching nothing. Piped output is the machine
    /// channel everywhere else in this tool (`fcli pr list | cut -f2` is real TSV), and the
