//! `fcli repo clone` — clone, then wire up the remotes a contributor actually needs.
//!
...
//! `origin` — is the repository `fcli` commands are about.
//!
//! That last step is the one that matters. Without it `fcli pr create` in a fresh fork clone
//! targets the fork, opening a pull request from a branch to itself; the resolver's remote-name
//! scoring (`upstream` 3, `origin` 1, see `forgejo_core::context`) is what fixes it, and this
...
use forgejo_core::{Result, types::RepoSlug};
...
Arguments after -- are passed to git clone.
  ...
  fcli repo clone forgejo/forgejo
  fcli repo clone my-thing ~/src/my-thing
  fcli repo clone forgejo/forgejo -- --depth 1 --filter=blob:none")]
pub struct Args {
...
fn clone(rt: &Runtime, args: &Args, slug: &RepoSlug, repo: &Repository) -> Result<()> {
    ...
    support::note(
        rt.term(),
        &format!(
            "{} added as {upstream}, and set as the base repository for fcli commands",
            parent.full_name
        ),
    );
    Ok(())
...
mod tests {
    ...
    /// Bug this prevents: `fcli repo clone o/r -- --depth 1` treating `--depth` as one of our own
