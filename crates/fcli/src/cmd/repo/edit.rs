//! `fcli repo edit` and `fcli repo rename`.
//!
...
and --disable-issues to turn features on or off.
  ...
  fcli repo edit --description 'A thing that does things'
  fcli repo edit --default-branch main --delete-branch-on-merge
  fcli repo edit them/proj --visibility private --disable-wiki")]
pub struct Args {
...
Git remotes are not updated. Forgejo redirects the old repository path.
  ...
  fcli repo rename better-name
  fcli repo rename them/proj new-name")]
...
pub fn run_rename(globals: &GlobalOpts, args: &RenameArgs) -> Result<()> {
    ...
    if name.contains('/') {
        return Err(Error::new(ErrorKind::Usage(format!(
            "repository name {name:?} cannot contain '/'. To change owners, use `fcli transfer`."
        ))));
    }
    crate::runtime::block_on(async move {
        let rt = Runtime::new(globals)?;
...
pub(crate) fn build(args: &Args) -> Result<EditRepoOption> {
    ...
    if body == EditRepoOption::default() {
        return Err(Error::new(ErrorKind::Usage(
            "no changes specified; see `fcli repo edit --help` for settings".to_owned(),
        )));
    }
    Ok(body)
}
...
mod tests {
    ...
    /// response `Presence` policy for request bodies, `fcli repo archive` starts turning off
    ...
    fn only_named_settings_reach_the_body() {
        ...
        assert_eq!(
            sent(&["fcli", "--description", "hi"]),
            serde_json::json!({"description": "hi"})
        );

        let body = sent(&["fcli", "--disable-wiki", "--visibility", "private"]);
    ...
    fn clearing_a_field_and_leaving_it_alone_are_different_bodies() {
        let cleared = serde_json::to_value(
            build(&args(&["fcli", "--description", "", "--website", ""])).expect("a body"),
        )
        .expect("serialisable");
        ...
        let untouched =
            serde_json::to_value(build(&args(&["fcli", "--enable-wiki"])).expect("a body"))
                .expect("serialisable");
        ...
        assert_eq!(build(&args(&["fcli", "--enable-wiki"])).unwrap().has_wiki, Some(true));
        assert_eq!(build(&args(&["fcli", "--disable-wiki"])).unwrap().has_wiki, Some(false));
        assert_eq!(build(&args(&["fcli", "--enable-issues"])).unwrap().has_wiki, None);
    }

    /// Bug this prevents: `fcli repo edit` with no flags reporting success while sending an empty
    ...
    fn an_edit_that_changes_nothing_is_a_usage_error() {
        let e = build(&args(&["fcli"])).unwrap_err();
    ...
    fn paired_toggles_refuse_to_be_given_together() {
        ...
        for pair in [
            ...
            assert!(
                <Harness as clap::Parser>::try_parse_from(["fcli", pair[0], pair[1]]).is_err(),
                "{pair:?}"
            );
        }
    }

    /// Bug this prevents: `fcli repo rename other/thing` being read as a rename *to* `other/thing`,
    ...
    fn a_new_name_with_a_slash_is_refused_with_the_right_advice() {
        ...
        assert!(e.to_string().contains("fcli transfer"), "{e}");
        assert_eq!(e.exit_code(), 2);
    }
}
