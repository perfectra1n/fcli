//! `fcli repo sync` — bring a fork up to date, locally or on the server.
//!
...
support -f.
  ...
  fcli repo sync                     # this checkout, from upstream
  fcli repo sync -b main -f
  fcli repo sync me/proj             # my fork, on the server
  fcli repo sync -s forgejo/forgejo  # from a specific base")]
pub struct Args {
...
fn refusal(info: &SyncForkInfo, slug: &RepoSlug, branch: Option<&str>) -> Option<Error> {
    ...
    Some(Error::new(ErrorKind::Usage(format!(
        "Forgejo will not sync {which}: it reports the sync as not allowed, which means the \
         ...
         sync this checkout instead (`fcli repo sync -b <branch> -f`) and push, or open a pull \
...
async fn local_sync(rt: &Runtime, globals: &GlobalOpts, api: &Api, args: &Args) -> Result<()> {
    ...
    // repository a `fcli pr create` here would target, so the two commands agree by construction.
...
mod tests {
    ...
    fn a_refusal_names_the_possible_causes_and_a_way_forward() {
        ...
        assert!(message.contains("fcli repo sync"), "{message}");
    }
    ...
    fn force_with_a_destination_is_refused_rather_than_ignored() {
        ...
        let h =
            <Harness as clap::Parser>::try_parse_from(["fcli", "me/proj", "-f"]).expect("parses");
        assert!(h.args.force);
