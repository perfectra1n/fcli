//! `fcli pr` — pull requests, including AGit.
//!
...
//! on when you leave it out. `fcli pr merge`, `fcli pr diff`, `fcli pr checks` with no arguments, in a
...
//!
//! **AGit is here.** `fcli pr create --agit` opens a pull request with no branch and no fork, by
//! pushing to `refs/for/<base>/<topic>`. That is a Forgejo capability with no GitHub equivalent — not
...
//!
//! **Two Forgejo shapes that surprise people**, both documented where they bite:
//!
//! * A **draft** is a `WIP:` title prefix, not a field. `--draft` and `fcli pr ready` are title
//!   edits. See [`common`].
