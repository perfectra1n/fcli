...
//!
//! Layer 2 (`fcli raw`) already reaches all 506 operations, so a command belongs here only if
...
//! `fcli raw` command doing the same job. That is deliberate: every endpoint already works, so
...
pub enum Porcelain {
    ...
    #[command(name = "auth")]
    Auth(auth::Args),
    /// Read and write fcli's configuration
    #[command(name = "config")]
    Config(config::Args),
...
impl Porcelain {
    pub fn run(&self, globals: &GlobalOpts) -> Result<()> {
        match self {
            ...
            Self::Quota(a) => quota::run(globals, a),
