...
//!   `fcli` could give a stuck user is a status column reading `waiting` forever.
...
#[derive(Debug, ClapArgs)]
pub struct ViewArgs {
    /// The run id, as printed by `fcli run list`
    pub run: RunId,
...
pub fn run(globals: &GlobalOpts, args: &Args) -> Result<()> {
    // Before the runtime, so `fcli run list --json` needs no host, no token and no network.
...
async fn list(rt: &Runtime, globals: &GlobalOpts, api: &Api, args: &ListArgs) -> Result<()> {
    ...
    if runs.iter().any(|r| is_queued(&r.status)) {
        support::note(
            rt.term(),
            "some runs are waiting for a runner. `fcli run view <id>` names the labels each job \
             asks for, and `fcli run runners` shows the labels your runners offer.",
        );
    }
...
/// `--exit-status`.
///
/// **Known wart.** A run that finished with `failure` is not an `fcli` error, and the taxonomy in
    ...
    if !is_finished(&run.status) {
        return Err(support::usage(format!(
            "run {} has not finished yet (status {}); --exit-status reports a conclusion, so \
             either wait for it with `fcli run watch {} --exit-status` or drop the flag",
...
/// command exists here rather than `fcli raw repo get-action-run-logs`.
async fn print_logs(
    rt: &Runtime,
    globals: &GlobalOpts,
    api: &Api,
    ...
    if wanted.is_empty() {
        ...
        return Err(support::usage(format!(
            "{why}; `fcli run jobs {}` lists the jobs and their ids",
...
///   `fcli`-managed handler would need tokio's `signal` feature, which this build does not
...
async fn warn_if_actions_disabled(rt: &Runtime, api: &Api, slug: &RepoSlug) {
    ...
    if !repo.has_actions {
        support::note(
            rt.term(),
            &format!(
                "Actions is disabled for {slug}. Enable it in repository settings or with `fcli repo edit --enable-actions` if supported."
            ),
        );
    }
}
...
mod tests {
    ...
    /// runner carries its label, with `fcli` reporting nothing but `waiting`.
    #[test]
    ...
    fn the_watch_interval_cannot_be_zero() {
        ...
        assert!(
            Harness::command()
                .try_get_matches_from(["fcli", "watch", "1", "--interval", "0"])
                .is_err()
        );
