//! `fcli auth` — get a token onto this machine, and prove it works.
//!
//! This group is the tool's front door. Every authentication diagnostic in `forgejo-core` ends
//! with "run `fcli auth login`", so nothing else in `fcli` is reachable for a new user until this
//! works.
//!
...
`fcli auth status` never prints tokens. Use `fcli auth token` to retrieve one.

  fcli auth login --host codeberg.org             # prompts for a token
  fcli auth login --host git.example.org --with-token < token.txt
  fcli auth status                                # per host: who you are, and whether it works
  fcli auth switch --host git.example.org         # change the active host or login
  fcli auth setup-git --host git.example.org      # let git push and pull with your fcli token";
...
pub enum Cmd {
    ...
    Token(token::Args),
    /// Configure git to authenticate with your fcli token
    ...
    /// git at whatever `fcli` the user actually invoked.
    #[command(hide = true)]
