//! `fcli config` — read and write `config.toml`.
//!
//! The key set is [`forgejo_core::config::KEYS`], not a list maintained here. That matters twice:
//! an unknown key is rejected with the real list rather than a guess, and a key added to
//! `forgejo-core` becomes settable with no edit to this file.
//!
...
use forgejo_core::config::{Config, HostKey, KEYS};
...
const LONG_HELP: &str = "\
Read and write fcli configuration.

`config get` prints the stored value, or nothing if unset. `config list` includes
defaults. Use --host for host-specific settings.

Manage aliases with `fcli alias`. Tokens are stored separately in the keyring
or hosts.toml, not config.toml.

  fcli config list
  fcli config get editor
  fcli config set editor 'code --wait'
  fcli config set pager cat --host git.example.org     # only for that host
  fcli config unset pager --host git.example.org";
...
/// path `fcli` itself reads through. So `config get` and `config unset` check here, and the message
...
fn checked_key(key: &str) -> Result<&str> {
    ...
    Err(support::usage(format!(
        "unknown config key {key:?}; the keys fcli recognises are: {}",
        KEYS.join(", ")
    )))
}
...
fn list(globals: &GlobalOpts, _args: &ListArgs) -> Result<()> {
    ...
    // `Config` preserves them on save and a user running an older fcli must not think a newer
