...
//!
//! So `fcli times add 42 1h25m` records an hour and twenty-five minutes you already spent, and
//! `fcli stopwatch start 42` begins measuring time you are about to spend. Neither can see the
//! other's state: a running stopwatch contributes nothing to a total until it is stopped.
//!
//! # What earns this a porcelain command over `fcli raw`
//!
...
Use `fcli stopwatch` for a running timer. Time is added to these totals only when
the timer stops. Durations require units: 1h25m, 90m, 2h, 45s, 1.5h, or 3d.

  fcli times add 42 1h25m              # record on issue #42 of this repository
  fcli times list 42                   # what has been recorded on #42
  fcli times list --total              # this repository's total, all issues, all users
  fcli times list --all --mine --total everything you have recorded, in every repository
  fcli times list --from 7d --user bob what bob logged here in the last week";
...
/// because this document is `fcli`'s own sum rather than anything the API returns, so there are
...
Include a duration unit: 1h25m, 90m, 2h, 45s, 1.5h, or 3d.
  ...
  fcli times add '#42' 90m --user bob")]
pub struct AddArgs {
...
totals contain duration, entries, and seconds.
  ...
  fcli times list 42     # one issue
  fcli times list        # every issue in this repository
  fcli times list --all  # every repository you can see — works outside a checkout")]
pub struct ListArgs {
...
Use the entry ID from `fcli times list`, not the issue number.")]
pub struct DeleteArgs {
    /// Issue the entry is on
    #[arg(value_name = "ISSUE")]
    pub issue: IssueIndex,

    /// Entry id, from `fcli times list`
    #[arg(value_name = "ID")]
...
pub fn run(globals: &GlobalOpts, args: &Args) -> Result<()> {
    // Before the runtime, so `fcli times list --json` answers with no token and no network.
...
async fn list(rt: &Runtime, api: &Api, globals: &GlobalOpts, args: &ListArgs) -> Result<()> {
    ...
    // `fcli times list | cut -f3` still sees only entries.
...
fn write_total(rt: &Runtime, globals: &GlobalOpts, times: &[TrackedTime]) -> Result<()> {
    ...
    // One line on stdout, not stderr: this *is* the answer, and `fcli times list --total`
    // belongs in a `$(…)`.
...
fn no_results(args: &ListArgs) -> String {
    ...
    format!(
        "No tracked time on {scope}. `fcli times add <issue> <duration>` records some, and \
         `fcli stopwatch start <issue>` times it live."
    )
}
        ...
        ErrorKind::ResourceNotFound { .. } | ErrorKind::RouteNotFound { .. } => {
            Error::new(ErrorKind::Usage(format!(
                "{slug} answered \"not found\" for a tracked-time request. Either the issue does \
                 ...
                 Settings ▸ Units ▸ Enable Time Tracking, or `fcli repo view --json \
...
mod tests {
    ...
    /// headed ISSUE, so `fcli times delete <that number>` operates on a different, real issue.
    #[test]
