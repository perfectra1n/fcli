//! `fcli repo` — repositories: create, clone, fork, view, list, edit, archive, sync.
//!
...
//! type out of `fcli raw`.
//!
//! Two Forgejo-specific things live here and nowhere else:
//!
...
//!   into one. So the flag belongs on `create`, and `fcli mirror` (a different group) covers push
...
pub enum Sub {
    ...
    Sync(sync::Args),
    /// Record which remote `fcli` should treat as the repository
...
/// `fcli repo view fcli` work — and is the one case that costs an extra request, so it is only
