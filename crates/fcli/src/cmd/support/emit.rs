...
impl Json {
    ...
    /// shape the command emits — which is not always the operation it calls: `fcli admin repo
...
impl<'a> Emit<'a> {
    ...
    /// on stderr for a terminal. `if fcli webhook list` must test reachability, not emptiness.
    ...
    ///
    /// stderr because stdout is the machine channel: `fcli topic add rust --json topics | jq`
