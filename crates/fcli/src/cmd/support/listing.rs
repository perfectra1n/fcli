...
pub enum Fields {
    ...
    Op(&'static str),
    /// A hand-written table, for output no single operation describes — `fcli workflow list`
    /// is assembled from the contents API, and `fcli search topics` unwraps a response
...
pub fn list_to(
    ...
    if triad.is_explicit() {
        // An empty collection is `[]` and exit 0, never an error: `if fcli run list --json id`
