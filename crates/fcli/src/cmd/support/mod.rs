...
use forgejo_core::error::{Error, ErrorKind};
...
pub fn writer(globals: &crate::global::GlobalOpts) -> forgejo_core::error::Result<Box<dyn Write>> {
...
pub fn api(rt: &crate::runtime::Runtime) -> forgejo_client::Api {
    forgejo_client::Api::new(rt.client().clone())
}
...
pub fn to_value<T: serde::Serialize + ?Sized>(
    value: &T,
) -> forgejo_core::error::Result<serde_json::Value> {
...
pub async fn resolve_me(
    ...
    values: &[String],
) -> forgejo_core::error::Result<Vec<String>> {
...
pub async fn me(api: &forgejo_client::Api) -> forgejo_core::error::Result<String> {
    Ok(api.user().get_current().await?.login)
}

/// Open a URL in the user's browser, honouring `fcli config get browser` and `$BROWSER`.
///
/// **Detached**, so `fcli pr view --web` returns immediately instead of blocking until the
...
pub fn open_web(rt: &crate::runtime::Runtime, url: &str) -> forgejo_core::error::Result<()> {
    if !rt.term().tty {
        // The URL is the answer when nobody is watching a browser: `fcli pr view --web | cat`
    ...
    result.map_err(|e| {
        usage(format!(
            "could not open a browser for {url}: {e}\nset one with `fcli config set browser <cmd>`"
        ))
    })
}
...
pub fn ago(ts: Option<&forgejo_core::types::Timestamp>) -> String {
    match ts {
...
///
/// stdout is the machine channel: `fcli topic add rust --json topics | jq` must not receive
...
/// write fails, so `fcli issue list 2>&1 | head -1` would abort the process on `EPIPE` instead
...
/// an empty stdout is then indistinguishable from success to `URL=$(fcli pr create --agit …)`,
