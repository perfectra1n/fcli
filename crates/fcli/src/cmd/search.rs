//! `fcli search` — instance-wide search for repositories, issues, pull requests, users and topics.
//!
...
//! That is a requirement from `docs/porcelain-conventions.md` (`fcli search` is named in it) and it
...
//! # `code` has no API
//!
//! Forgejo 16.0.4 has no code-search endpoint at all: the instance's `/explore/code` page is web
...
use forgejo_model::{Issue, Repository, TopicResponse, User};
...
///
/// Words rather than one string so `fcli search repos anvil factory` works without quoting, which
/// is how people type a search. An empty query is legal and means "everything", which is the
/// natural reading of `fcli search repos --owner acme`.
...
fn code(rt: &Runtime, args: &CodeArgs) -> Result<()> {
    ...
    if args.web {
        let url = format!(
            "{}/explore/code?q={}",
            rt.client().web_base(),
            forgejo_core::http::encode::query(&query)
        );
...
fn no_code_api(query: &str) -> Error {
    Error::new(ErrorKind::Usage(format!(
        "Forgejo code search is only available in the web interface. Open it with:\n  fcli search code {query} --web\nor search repository names and descriptions:\n  fcli search repos {query}"
    )))
}
...
mod tests {
    ...
    /// first. Bug this prevents: sending `uid=acme`, which Forgejo answers with an empty result set
    ...
    fn code_search_explains_the_missing_api() {
        ...
        assert!(msg.contains("fcli search repos"), "{msg}");
    }
