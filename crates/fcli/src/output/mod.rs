...
mod tests {
    ...
    /// unprojected document, and `fcli` would disagree with `gh` on a flag combination users
    /// actually type.
    #[test]
    ...
    fn binary_to_a_tty_is_refused_unless_forced() {
        ...
        assert!(guard_binary("application/x-forgejo-blob", &tty, &Dest::Stdout, false).is_err());
    }
