...
//! mechanical edit *here* and nowhere else, because the rest of `fcli` sees exactly two
...
///
/// The whole `jaq` surface `fcli` uses is the two inherent methods below.
pub struct Filter {
    ...
    match v {
        ...
        Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                ...
                // Forgejo has two `uint64` fields; they exceed `i64` only at absurd values,
