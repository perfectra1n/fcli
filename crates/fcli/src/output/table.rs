...
//!
//! There is exactly one `Table` in `fcli`, and both the default human renderer and the
...
//!
//! The right-hand column is the contract that makes `fcli pr list | cut -f2` work, and it is
...
impl Table {
    ...
    /// for its resource (and whether `M` is even known — the Forgejo API does not always send
...
mod tests {
    ...
    /// `fcli pr list | cut -f2`; with padding that yields `open   ` and with a header it
