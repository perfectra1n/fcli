//! Library half of the `fcli` binary.
//!
...
//!
//! `main.rs` should reach this code as `fcli::output::…` rather than declaring `mod output;`
...
/// The one-line description, shared by both parse phases so `fcli --help` and
/// `fcli raw … --help` cannot describe two different tools.
...
Choose a command:
  ...
  fcli api <endpoint>        any REST endpoint, by path
  fcli raw <group> <op>      commands generated from the API specification
  fcli <noun> <verb>         common tasks such as pr list and repo clone

`fcli raw search <words>` finds an operation when you do not know its name.";
...
/// in `fcli --version` and in the `User-Agent` is the same value that stamped the generated
...
/// `fcli 0.1.0 (Forgejo API 16.0.4)` — the version string both parse phases hand to clap.
