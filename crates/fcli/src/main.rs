//! `fcli` — a Forgejo command-line client.
//!
//! # The two-phase parse
//!
//! Layer 2 is all 506 API operations as clap commands. Building that tree eagerly would cost
//! thousands of `Arg` allocations on **every** invocation, `fcli --version` included: five to
...
//!
//! * [`fcli_raw::peek`] compares a few `OsStr`s. If the first non-flag word is not `raw` (or its
...
//!   `fcli raw --help`, one full command for `fcli raw repo create-pull-request`.
//! * Everything else goes through the small derive tree.
//!
//! Both roots carry the same global flags, from [`fcli::global`]. Layer 2's root is built here
//! rather than by [`fcli_raw::build`] for exactly that reason: the globals belong to the binary,
...
use fcli::exit::{self, Fail};
use fcli::global::{self, GlobalOpts};
use fcli::{ABOUT, LONG_ABOUT};
use forgejo_client::meta::OPS;
use forgejo_client::meta_types::lookup;
...
/// with "failed printing to stdout". `fcli pr list | head -1` is a completely ordinary thing to
...
#[derive(Parser)]
#[command(name = "fcli", about = ABOUT, long_about = LONG_ABOUT, disable_help_subcommand = true)]
struct Cli {
    #[command(subcommand)]
    command: Cmd,
}
...
#[derive(Subcommand)]
enum Cmd {
    /// Call any Forgejo REST endpoint by path
    #[command(long_about = API_LONG_ABOUT)]
    Api(fcli::api::ApiArgs),

    /// Call any Forgejo API operation directly
    ///
    /// Present so that `fcli --help` lists it. Real `fcli raw …` invocations never reach here:
    ...
    /// The layer-3 groups, flattened in so that `fcli pr` sits beside `fcli api` rather than
    /// under a wrapper noun.
    #[command(flatten)]
    Porcelain(fcli::cmd::Porcelain),
}
...
JSON types and reads a file when the value starts with @ (@- is stdin).
  ...
  fcli api -i repos/o/r";

fn dispatch(argv: &[OsString]) -> Result<(), Fail> {
    // User aliases are expanded **before** either parse phase, because `fcli prs` is not a
    ...
    // `fcli --remote origin pr list`: parsed, and never run. Checked before the command is
    ...
    match cli.command {
        Cmd::Api(args) => {
            ...
            fcli::api::run(&globals, &args, sub).map_err(Fail::from)
        }
        Cmd::Porcelain(p) => {
        ...
        Cmd::Raw { .. } => {
            let peeked =
                fcli_raw::peek(argv).unwrap_or(fcli_raw::Peeked { group: None, leaf: None });
...
/// Id of the hidden root-level `--remote`. Prefixed like [`fcli::global`]'s so it cannot collide
/// with a generated layer-2 id.
const REMOTE_NUDGE: &str = "fcli:remote-nudge";

/// The hidden `--remote`, **deliberately not `.global(true)`**.
///
/// # Why it cannot be a global
///
/// `tea`'s `-R` is `--remote` and takes a git remote *name*; `fcli`'s `-R` is `--repo` and takes
...
///
/// The obvious implementation — put `--remote` in [`fcli::global::args`] alongside `--repo` — is a
...
///
/// ```text
/// fcli pr create --remote <name>     # the remote to push through
/// fcli repo create --remote <name>   # the remote to add to --source  (also -r)
/// fcli repo fork --remote            # a boolean: rewire this checkout
/// ```
///
/// A global copy would crash all three on an ordinary command line — the same failure
/// `crates/fcli/tests/porcelain_cli.rs` was written to catch after it happened three times in one
...
/// `fcli --remote origin pr list`, and [`is_unknown_remote`] catches the flag appearing after a
...
fn remote_nudge(given: Option<&str>) -> forgejo_core::Error {
    let named = given.filter(|g| !g.is_empty());
    let example = named.map(|g| format!(" (you gave {g:?}, which is a git remote name)"));
    forgejo_core::Error::new(forgejo_core::ErrorKind::Usage(format!(
        "--remote is not supported here{}. -R/--repo takes owner/name, for example \
         `fcli -R myorg/myrepo pr list`. To select a remote for this checkout, run \
         `fcli repo set-default`. --remote is available on pr create, repo create, and repo fork.",
...
fn layer2(argv: &[OsString], group: Option<&str>, leaf: Option<&str>) -> Result<(), Fail> {
    // The root is built by `fcli::raw::root` so that the 506-operation smoke test builds exactly
    // what runs here — see the comment there on why a duplicate flag name would be a panic.
    let matches = fcli::raw::root(group, leaf).try_get_matches_from(argv)?;
    ...
    let op = lookup::op(OPS, group_name, command_name).ok_or_else(|| {
        ...
        forgejo_core::Error::new(forgejo_core::ErrorKind::Usage(format!(
            "`{group_name} {command_name}` parsed but is not in the operation table; \
             this is a bug in fcli — please report it"
        )))
    })?;

    // Globals may appear at any of four depths: `fcli --host x raw repo get`,
    // `fcli raw --host x repo get`, `fcli raw repo --host x get`, `fcli raw repo get --host x`.
    let globals = GlobalOpts::from_chain(&[leaf_matches, group_matches, raw_matches, &matches]);
    fcli::raw::run(&globals, op, leaf_matches).map_err(Fail::from)
}

/// `fcli raw search <words>`: the answer to "506 commands are undiscoverable".
fn search(m: &clap::ArgMatches) -> Result<(), Fail> {
    use std::io::Write;
    let terms: Vec<String> =
        m.get_many::<String>("terms").map(|v| v.cloned().collect()).unwrap_or_default();
    let hits = fcli_raw::search::search(OPS, &terms, fcli_raw::search::DEFAULT_LIMIT);
    let text = fcli_raw::search::render(&hits, &terms);
    let mut out = std::io::stdout().lock();
    out.write_all(text.as_bytes()).map_err(|e| Fail::from(forgejo_core::Error::from(e)))?;
    out.flush().map_err(|e| Fail::from(forgejo_core::Error::from(e)))?;
    Ok(())
}
...
mod tests {
    ...
    fn the_hidden_remote_nudge_does_not_collide_with_the_real_per_command_remote() {
        ...
        for argv in [
            &["fcli", "pr", "create", "--remote", "upstream", "--fill"][..],
            &["fcli", "repo", "create", "widget", "--remote", "origin"][..],
            &["fcli", "repo", "create", "widget", "-r", "origin"][..],
            &["fcli", "repo", "fork", "--remote"][..],
        ] {
    ...
    /// `fcli --remote origin pr list` parses — and is then refused with the nudge, before any
    /// command runs.
    #[test]
    fn a_leading_remote_is_answered_with_the_repo_flag_instead_of_silence() {
        let m = root()
            .try_get_matches_from(["fcli", "--remote", "origin", "pr", "list"])
    ...
    /// `fcli pr list --remote origin` — where a tea user actually puts it. The flag is not an
    ...
    fn a_trailing_remote_on_a_command_that_has_none_is_recognised() {
        for argv in [
            &["fcli", "pr", "list", "--remote", "origin"][..],
            &["fcli", "issue", "list", "--remote=origin"][..],
            &["fcli", "repo", "view", "--remote", "upstream"][..],
        ] {
        ...
        let e = root()
            .try_get_matches_from(["fcli", "pr", "list", "--remotely"])
            .expect_err("clap must refuse it");
        assert!(!is_unknown_remote(&e), "{e}");
        let e = root()
            .try_get_matches_from(["fcli", "pr", "list", "--nosuchflag"])
            .expect_err("clap must refuse it");
        assert!(!is_unknown_remote(&e), "{e}");
    }
    ...
    fn the_nudge_never_fires_where_remote_is_real() {
        let ok = root().try_get_matches_from(["fcli", "pr", "create", "--remote", "upstream"]);
    ...
    /// `fcli --help` would grow five entries `gh` does not have.
    #[test]
    fn the_tea_aliases_are_not_subcommands_of_the_root() {
        let root = root();
        for (name, _) in fcli::cmd::alias::BUILTIN {
            assert!(
    ...
    fn every_tea_alias_expands_to_something_the_root_accepts() {
        let root = root();
        for (name, expansion) in fcli::cmd::alias::BUILTIN {
            let mut argv = vec!["fcli".to_owned()];
