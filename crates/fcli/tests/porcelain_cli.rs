...
//!
//! That is not hypothetical. Writing this test found three live crashes in the group this file was
//! added with:
//!
//! ```text
//! fcli admin repo create   --template collides with the global --template
//! fcli admin repo list     -q         collides with the global -q (--jq)
//! fcli admin quota rule …  --limit    collides with the global --limit
//! ```
//!
...
/// with a global is possible, so this reconstructs it from the public `fcli::cmd::Porcelain`.
#[derive(Parser)]
#[command(name = "fcli")]
struct Root {
    #[command(subcommand)]
    _command: fcli::cmd::Porcelain,
}
...
/// Bug this prevents: `fcli admin repo create --owner x widget` panicking inside clap with
...
fn walk(cmd: &clap::Command, path: &str, reserved: &[&str], offenders: &mut Vec<String>) {
    for arg in cmd.get_arguments() {
        ...
        {
            offenders.push(format!("  fcli {path}: --{long} (argument id {id})"));
        }
    }
...
/// Every group and subcommand needs an `about`, because `fcli <group> --help` is how the tool is
...
fn the_administration_and_plumbing_groups_are_reachable_under_their_command_names() {
    ...
    for (group, verbs) in [
        ...
        ("admin", &["user", "org", "repo", "cron", "adopt", "runner", "email", "quota"][..]),
    ] {
        let cmd = root
            .get_subcommands()
            .find(|c| c.get_name() == group)
            .unwrap_or_else(|| panic!("fcli {group} is missing"));
        for verb in verbs {
            assert!(
                cmd.get_subcommands().any(|c| c.get_name() == *verb),
                "fcli {group} {verb} is missing"
            );
        }
    }
}

/// `fcli reaction add +1` must not be readable as a flag.
///
...
fn a_reaction_named_minus_one_parses_as_a_value_not_a_flag() {
    for argv in [
        &["fcli", "reaction", "add", "-1", "--issue", "42"][..],
        &["fcli", "reaction", "add", "--issue", "42", "-1"][..],
        &["fcli", "reaction", "remove", "-1", "--comment", "918273"][..],
...
fn webhook_scopes_are_mutually_exclusive() {
    let ok = tree().try_get_matches_from(["fcli", "webhook", "list", "--global"]);
    assert!(ok.is_ok(), "{:?}", ok.err().map(|e| e.to_string()));

    for pair in [["--org", "acme"], ["--user", "--global"], ["--global", "--org"]] {
        let mut argv = vec!["fcli", "webhook", "list"];
        argv.extend(pair);
        argv.push("--user");
        assert!(
...
/// `fcli reaction list` with neither target must be a *parse* error naming both flags, not a
...
fn reaction_requires_exactly_one_target() {
    assert!(tree().try_get_matches_from(["fcli", "reaction", "list"]).is_err());
    assert!(
        tree()
            .try_get_matches_from(["fcli", "reaction", "list", "--issue", "1", "--comment", "2"])
            .is_err()
    );
    assert!(tree().try_get_matches_from(["fcli", "reaction", "list", "--issue", "1"]).is_ok());
}
