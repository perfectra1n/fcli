...
//! including `fcli --version` — 5–20 ms of work to build a parser the user never reaches, in a
...
//! | --- | --- |
//! | `fcli --version` | nothing — [`peek`] returns `None` |
//! | `fcli raw --help` | one stub per group (~12 `Command`s, no `Arg`s) |
//! | `fcli raw repo --help` | one name-and-`about` stub per op in `repo` |
//! | `fcli raw repo get --help` | exactly one full `Command`, with its args |
//!
...
pub const ID_BODY_FILE: &str = "fcli:body-file";
pub const ID_DRY_RUN: &str = "fcli:dry-run";
pub const ID_PAGINATE: &str = "fcli:paginate";
pub const ID_LIMIT: &str = "fcli:limit";
...
pub struct Peeked<'a> {
    /// The group token, if one was present. `None` for `fcli raw` / `fcli raw --help`.
...
///
/// Without this list, `fcli --host example.org raw repo get` would see `example.org` as the
...
/// `fcli x repo get` peeks the same subtree it will later parse.
pub fn peek(argv: &[OsString]) -> Option<Peeked<'_>> {
    ...
    // Only look for a leaf once a group is known: `fcli raw --help` has neither.
...
/// Build the root `fcli` command carrying only the requested layer-2 subtree.
///
...
pub fn build(
    ...
    Command::new("fcli").subcommand(raw_command(ops, groups, group, leaf))
}
...
pub fn raw_command(
    ...
    let mut raw = Command::new("raw")
        ...
        // `.visible_alias` would.
        .alias("x")
        .about("Call any Forgejo API operation directly")
        .long_about(
            "Commands for every operation in the bundled Forgejo API specification.\n\n\
             ...
             `fcli raw search <words>` to find an operation by name, summary, or URL path.",
        )
...
/// A leaf with a name and an `about` and no arguments: enough for `fcli raw repo --help` to
...
/// Forgejo version while the instance may be newer and accept values this build has never
/// heard of; rejecting them here would make `fcli` refuse requests the API would honour, and
/// the user would have no way around it short of dropping to `fcli api`. Being permissive
...
mod tests {
    ...
    fn peek_recognises_the_three_depths_of_a_raw_invocation() {
        assert_eq!(peeked(&["fcli", "raw"]), Some((None, None)));
        assert_eq!(peeked(&["fcli", "raw", "repo"]), Some((Some("repo".into()), None)));
        assert_eq!(
            peeked(&["fcli", "raw", "repo", "create-pull-request"]),
    ...
    fn peek_declines_non_raw_invocations() {
        assert_eq!(peeked(&["fcli", "--version"]), None);
        assert_eq!(peeked(&["fcli", "pr", "list"]), None);
        assert_eq!(peeked(&["fcli"]), None);
    }
    ...
    fn peek_skips_global_flags_and_their_values() {
        assert_eq!(
            peeked(&["fcli", "--host", "x", "raw", "repo", "get"]),
    ...
    /// `fcli raw search <words>` must exist whenever no group was named, or discoverability
    ...
    fn search_is_reachable_from_the_bare_raw_command() {
        ...
        assert!(
            cmd.clone().try_get_matches_from(["fcli", "raw", "search", "pull", "request"]).is_ok()
        );
    }
    ...
    fn enum_values_are_a_hint_and_do_not_reject_unknown_input() {
        ...
        let m = cmd.try_get_matches_from(["fcli", "raw", "issue", "list", "--state", "bananas"]);
...
mod generated_table {
    use super::*;
    use forgejo_client::meta::{GROUPS, OPS};
    ...
    fn every_generated_op_builds_parses_and_renders_help() {
        ...
        for op in OPS {
            ...
            let mut argv: Vec<String> =
                vec!["fcli".into(), "raw".into(), op.group.into(), op.command.into()];
