//! `fcli raw search <terms>` — the answer to "506 commands are undiscoverable".
//!
...
use forgejo_client::meta_types::{BodyField, OpMeta};

/// How many hits `fcli raw search` prints. Fifteen fits a terminal without a pager.
...
/// The `search` leaf, built alongside the group stubs so `fcli raw search …` needs no group.
    ...
    clap::Command::new("search")
        ...
        .long_about(
            "Rank all operations against the given words and print the best matches as full \
             invocations, ready to copy.\n\nExample: fcli raw search pull request",
        )
        .arg(
            clap::Arg::new("terms")
                .value_name("WORDS")
...
pub fn invocation(op: &OpMeta) -> String {
    let mut s = format!("fcli raw {} {}", op.group, op.command);
...
pub fn render(hits: &[Hit<'_>], terms: &[String]) -> String {
    if hits.is_empty() {
        return format!(
            "no operation matches {:?}\n\ntry fewer or different words, or `fcli raw --help` \
...
mod tests {
    ...
    fn an_invocation_shows_path_placeholders_and_required_body_flags() {
        ...
        assert_eq!(invocation(op), "fcli raw repo create-pull-request <OWNER> <REPO> --title …");
        let op = crate::fixtures::op("repo", "download-pull-diff-or-patch");
        assert_eq!(
            invocation(op),
            "fcli raw repo download-pull-diff-or-patch <OWNER> <REPO> <INDEX> <DIFF_TYPE>"
        );
    }
    ...
    fn no_match_says_what_to_do_next() {
        ...
        assert!(text.contains("fcli raw --help"), "{text}");
    }
    ...
    fn rendered_hits_show_the_method_and_path() {
        ...
        let text = render(&search(OPS, &terms, DEFAULT_LIMIT), &terms);
        assert!(text.contains("fcli raw repo get-contents <OWNER> <REPO> <FILEPATH>"), "{text}");
